...
**Current fix round:** v1.3 "Forward" — **first native TLS 1.3 on Classic Mac OS, ever.** macTLS gains a hand-written TLS 1.3 handshake, key schedule, and record layer per RFC 8446 on top of BearSSL primitives (BearSSL itself has no 1.3). X25519 key exchange, ChaCha20-Poly1305 (`0x1303`) and AES-128-GCM (`0x1301`) cipher suites, SHA-256 transcript, server auth against the 121-anchor Mozilla CCADB bundle. Every connection opens a 1.3 ClientHello that also advertises 1.2 suites; falls back to BearSSL's full 1.2 path if the server declines 1.3. The switch is transparent through `OSTLS_New/Start/Pump/Read/Write`. Verified end-to-end on a G3 iMac (OS 9.2.2) by Cloudflare's `/cdn-cgi/trace` (`tls=TLSv1.3, kex=X25519`), Akamai's TLS 1.3 endpoint (`TLS_CHACHA20_POLY1305_SHA256`), BrowserLeaks (full handshake fingerprint), and How's My SSL (forward secrecy + no insecure ciphers + no TLS compression). Adapted from Certainly (minorbug, MIT) C99 → CW8 C89.
**Predecessor:** v1.2 "Sealed" (2026-05-29, ~6 hours earlier) — macEntropy v1.0 closed the entropy hole, POST forms wired through both fetchers, V1 download manager landed, plus #41 / #56 / #36 / #150.
**Last release:** **MacSurf v1.3 "Forward"** (2026-05-29). Full notes: [release-notes/MacSurf-1.3.md](release-notes/MacSurf-1.3.md). Predecessor v1.2 notes: [release-notes/MacSurf-1.2.md](release-notes/MacSurf-1.2.md).
**Last hardware-accepted:** v1.3 TLS 1.3 (2026-05-29).

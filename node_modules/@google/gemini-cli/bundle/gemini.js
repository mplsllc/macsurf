...
async function handleList2(args) {
  ...
  process.stdout.write(import_chalk4.default.bold("Discovered Agent Skills:") + "\n\n");
  for (const skill of skills) {
    ...
    process.stdout.write(
      `${import_chalk4.default.bold(skill.name)} ${status}${builtinSuffix}
`
    );
...
async function loadCliConfig(settings, sessionId2, argv, options = {}) {
  ...
  const useGeneralistProfile = settings.experimental?.generalistProfile ?? false;
  const useContextManagement = settings.experimental?.contextManagement ?? false;
...
function entrypoint(workdir, cliArgs) {
  ...
  const cliCmd = false ? isDebugMode2 ? "npm run debug --" : "npm rebuild && npm run start --" : isDebugMode2 ? `node --inspect-brk=0.0.0.0:${process.env["DEBUG_PORT"] || "9229"} $(which gemini)` : "gemini";
...
`);
    ...
    try {
      ...
      const reconstructFatalError = (event) => {
        const errorMeta = event._meta;
        const name = typeof errorMeta?.["errorName"] === "string" ? errorMeta["errorName"] : void 0;
        let errToThrow;
        switch (name) {
          ...
          case "tool_response": {
            ...
            if (streamFormatter) {
              ...
              streamFormatter.emitEvent({
                ...
                output: displayText,
                error: event.isError ? {
                  type: typeof event.data?.["errorType"] === "string" ? event.data["errorType"] : "TOOL_EXECUTION_ERROR",
                  message: errorMsg
                } : void 0
              });
            }
...
`);
              ...
              handleToolError(
                ...
                new Error(errorMsg),
                config,
                typeof event.data?.["errorType"] === "string" ? event.data["errorType"] : void 0,
                displayText
              );
            }
            break;
...
`);
          ...
          case "agent_end": {
            if (event.reason === "aborted") {
              ...
              const isConfiguredTurnLimit = typeof event.data?.["maxTurns"] === "number" || typeof event.data?.["turnCount"] === "number";
...
function getNodeMemoryArgs(isDebugMode2) {
  ...
  const isV8SandboxEnabled = (
    ...
    process.config?.variables?.v8_enable_sandbox === 1
  );
  if (isV8SandboxEnabled && !process.execArgv.some(
    (arg) => arg.startsWith("--max-external-pointer-table-size")
  )) {
    args.push(eptFlag);
  }

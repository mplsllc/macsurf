...
- Last shipped fix: **fixes135b** — **text-overflow ellipsis visual truncation.** Redraw-time prefix-fit-and-append-marker in [html_redraw_text_box](browser/netsurf/content/handlers/html/redraw.c). New helper `html_redraw_text_overflow_ellipsis_active(box)` walks parent chain for a styled ancestor with `text-overflow: ellipsis` + `overflow-x != visible`. Trigger condition: `(x + box->width > clip->x1)` AND helper returns true. Binary-search byte-stepped prefix fit against `clip->x1 - x - ellipsis_width`. Marker is UTF-8 `…` (folds to MacRoman 0xC9 on Mac); falls back to ASCII `.` if measurement fails. Output written to a 512-byte stack buffer and passed to text_redraw in place of `box->text`. Single file: redraw.c.

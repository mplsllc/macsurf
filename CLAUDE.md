...
- Last shipped fix: **fixes135a** — **`text-overflow` libcss plumbing.** New libcss property `text-overflow: clip | ellipsis` parses, cascades, and computes. New files: [p_text_overflow.c](browser/libcss/src/parse/properties/p_text_overflow.c), [s_text_overflow.c](browser/libcss/src/select/properties/s_text_overflow.c). Bit slot at word 15, shift 5, mask 0x60 (2 bits for INHERIT/CLIP/ELLIPSIS). Public accessor `css_computed_text_overflow()` added in [computed.c](browser/libcss/src/select/computed.c). `dispatch.c` and `s_dispatch.c` both updated to add the new slot (the dual-dispatch sync rule from fixes116 still required). Test probes in `tests/css/text_overflow.html` and at `mactrove.com/advanced.html`. Phase A only — visual truncation arrives in fixes135b.

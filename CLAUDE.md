...
- Last shipped fix: **fixes117** — **native CSS `grid-template-columns` track widths (PX / FR / PERCENT).** Round B from the audit table. The fixes115 preprocessor still rewrites `grid-template-columns` to `-macsurf-grid:` at the byte level, but now emits **the actual track list** (e.g. `-macsurf-grid: 210px 1fr` instead of `-macsurf-grid: 2`). `p_macsurf_grid.c` accepts a V2 track-list grammar (dimension / percentage / number tokens) and emits 8 int32 track descriptors after the existing packed `cols<<16|rows` value. Each track packs `unit (bits 31..28) | value (bits 27..0)`: PX in pixels, FR/PERCENT in Q20.8. Storage = new inline `int32_t macsurf_grid_tracks[8]` in `css_computed_style_i` (32 bytes per distinct computed style, ~10KB total per page). `layout_grid.c` distributes width by subtracting PX + PERCENT widths from the container, then dividing the remainder across FR tracks proportional to their Q8.8 ratio. Legacy `-macsurf-grid: N` integer form is preserved unchanged; tracks[0] == 0 triggers the equal-width fallback. **MacSurf.mcp:** no new files. **Sites affected:** any responsive layout using `grid-template-columns: 210px 1fr`, `repeat(N, 1fr)`, `1fr 2fr`, etc. Mactrove's main 2-column sidebar should now render with a real 210px sidebar instead of collapsing to equal cells.

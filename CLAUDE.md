...
**The working tree is at 2.0 — trust `git status`, not prose.** Tip is **fixes765** (`8cad5551`, `master`), pushed to origin + github. **MacSurf 2.0 is being cut now:** every version string is bumped (About box / default UA / `navigator.userAgent` / session banner / new Finder `'vers'` resource all report **2.0**), the release notes are written ([docs/release-notes/MacSurf-2.0.md](docs/release-notes/MacSurf-2.0.md)), and the Read Me (`readme.txt`), macintoshgarden blurb (`.private/macintoshgarden-description.html`), and both websites (macsurf-web / macsurf-home) are refreshed. Remaining MANUAL steps: user builds `MacSurf20.sit` from the current tree, drops it at `macsurf-web/builds/MacSurf20.sit`, then tags `v2.0` + cuts the GitHub release. Last SHIPPED/tagged release was **1.68.2 "macQJS"** (`v1.68.2`, `d2eee8e3`, `MacSurf1682.sit`); **2.0 absorbs everything since — the whole fixes688→765 batch**, including the never-shipped 1.68.3 "Blank Page" work.
...
**Remotes: origin + github ONLY. Codeberg was DROPPED 2026-07-10** ([[feedback_hold_pushes_until_tested]]). Working branch is `master`; `password-manager` and `TwoPointOh` are stale/behind — leave them until an explicit merge. **Diagnostic logging is now failures-only by default** (fixes765): the crash-only gate keeps banner + NAV + `RECON HEAP BOUNDS` + genuine FAIL/exception/crash lines; re-enable the full trace with `MACSURF_NUCLEAR_LOG` in `macsurf_debug_log.c` when chasing a new bug.

...
- **Highest-value remaining CSS (post fixes834), roughly ranked:**
  - **`appearance` + form-control styling (#80/#90)** — the one genuine multi-file refactor: form controls render via Carbon Control Manager (`Draw1Control`); `appearance:none` means synthetic CSS-painted controls + new click/key handling. Visible on every form. Own dedicated round.
  - **`min-content`/`max-content`/`fit-content` intrinsic-sizing solver** — the structural prerequisite that unblocks `table-layout:auto` and correct flex/grid shrink-to-fit. High effort, unblocks a lot.
  - **`background-clip: text`** (gradient headings) — the flashy part of #255 deferred; needs glyph-clip painting. `background-origin` is the cheap companion (same fixes832 plumbing).
  - **`justify-self`** (per-item grid override, #279) — same pattern as justify-items BUT the `bits[16]` array is FULL, so it needs the array extended first (see the native-property-add gotcha).
  - **Grid Round 2 (#279 remainder):** placement/span-aware auto sizing, minmax composition, §12.8 stretch default.
- **`#277` inline-rewrite completeness** — fixes831 added the major rewriters to the inline path; audit for any still-skipped (grid placement longhands, etc.).
...
- **Other parsed-but-silently-dropped gaps:** `unicode-bidi`, `writing-mode`, `break-*`, `page-break-*`, `orphans`, `widows`, `fill-opacity`, `stroke-opacity`, `clip-path`, `mask`, `filter`, multi-tier `font-weight` (platform limit, QuickDraw is bold/regular only).
- **`transition` / `animation` are NOT in that category — they are never parsed at all.** There is no `CSS_PROP_TRANSITION*` / `CSS_PROP_ANIMATION*` enum anywhere in libcss; the shorthand and every longhand are **deleted from the CSS text by the preprocessor** before libcss sees a byte, via the `DROP_PROPS[]` table in [content/handlers/css/cssh_css.c](browser/netsurf/content/handlers/css/cssh_css.c) (~:3766-3785, fixes191f — they drop together so a stray ident can't trip the parser). Consequence for anyone implementing them: this is a **from-scratch native property add** (the ~15-file template below) plus a timer-driven playback layer, not "wire up a consumer for a value libcss already computed." #34 OPEN.

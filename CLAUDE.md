...
**The working tree is CLEAN and FULLY COMMITTED — trust it.** Tip is **fixes696** (`45d4d9b3`) on `master`, pushed to all three remotes (origin/github/codeberg). Shipped/tagged release is still **MacSurf 1.68.2 "macQJS"** (GitHub release `v1.68.2`, tag on `d2eee8e3`, asset `MacSurf1682.sit`). Since 1.68.2, commits fixes688→696 are on master but **NOT yet hardware-verified** — a batch of perf + TLS + chrome work whose issues stay OPEN pending a G3 pass (see the verify queue below). There are **no untracked build files and no pending renames** — everything in the CW8 build is tracked in git. `TwoPointOh` is a deliberate fallback branch, currently behind master; leave it until an explicit merge. (When editing this banner, re-run the state check — see [Working-tree hygiene](#working-tree-hygiene) — so it never goes stale again.)

**Post-1.68.2 verify queue (code committed, awaiting G3):** #206 macintoshgarden HTTPS/HRR (fixes691), #208 reflow storm (fixes689), #209 JS-interrupt WNE throttle (fixes690), #210 querySelector first-match (fixes690), #47 History menu (fixes694), #50 bookmark-folder model foundation (fixes693), #153 dead-proxy-code removal (fixes692). fixes695 = libdom `.h`→`_i.h` internal-header rename + libwapcaplet bool-pin (CW8 flat-namespace collision fix); fixes696 = tracked two previously-untracked frontend build files (`time.h` shim, `macos9_desktop_stubs.c`). **Mac side: update MacSurf.mcp for the libdom `_i.h` renames and confirm the two fixes696 files are in the project.** #211 (per-node wrapper perf) is the one perf issue still un-coded.

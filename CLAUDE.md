...
- **v0.4.6 IMAGES on G4 / OS 9.2.2 (2026-05-16).** Reached at fixes78r. All five image formats — **PNG / GIF / BMP / TIFF / JPEG** — decode and render via QuickTime Graphics Importers. Verified on `advanced.html` PROBE I1–I5. **GIF gets per-pixel transparency for free** because QT's GIF importer honours the file's palette transparency at decode time. **PNG / TIFF render opaque** — QT's `QTNewGWorld(k32ARGBPixelFormat)` returns `cDepthErr (-157)` on this Carbon-era QuickTime build, meaning the OSType-aware pixmap allocation path doesn't exist on OS 9 (it was a QuickTime 6 / Mach-O addition that never landed on this platform). The fixes78r fallback degrades alpha-format decodes to vanilla `NewGWorld(32)` + opaque render so they're at least visible. Real PNG / TIFF per-pixel alpha is gated on **Path A**: linking NetSurf's core image handlers (`content/handlers/image/{png.c, gif.c, jpeg.c}`) + libpng / libnsgif / libjpeg, which decode straight into the frontend bitmap with real RGBA. Atari / RISC OS frontends use this pattern — none of them write a custom `content_handler` for images, they only provide `gui_bitmap_table`.
- Last shipped fix: **fixes78r** — **graceful degradation when QTNewGWorld ARGB is rejected.** Detects `cDepthErr` from `QTNewGWorld(k32ARGBPixelFormat)` and falls back to vanilla `NewGWorld(32)` with `wants_alpha` flipped to false, so the image still decodes and renders (opaquely). Two-bg-pass diff (fixes78o-q) is gone; the magenta-keyed `transparent` CopyBits path in `plotters.c` remains for the day per-pixel alpha lands via Path A. Logs both QTNewGWorld error codes so the rejection class is recorded in the file log.

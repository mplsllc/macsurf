# ⚠️ READ THIS FIRST: MacSurf STATE (updated 2026-07-16)

**Trust `git status` + the git facts below, not prose.** **The active branch is `master` (2026-07-17).** `css-coverage` and `revamp` were merged into `master` (all fast-forward) and then **DELETED** on local + origin + github — master is now the sole development line. HEAD is at the 2.0.5 release batch (fixes876-894), which includes the whole post-2.0 CSS-coverage work, the Facebook Round 2 / JS-engine work (835-875), the revamp-plan Phase 0/1 batch (876-893), and **fixes894 = a revert of fixes884/885** (the cert-failure-no-cleartext gate broke bare-domain→http→https redirects like hackernews.com; reverted for the release, to be fixed forward). Pushed to origin + github. The remotes now carry ONLY `master` + the stale `password-manager`.
...
- **XenForo reply/editor page (tinkerdifferent.com) fully non-functional — traced to a single upstream DOM-shim cascade, NOT a Froala size issue.** (2026-07-05, unfixed, next session's target.) Confirmed via `js/xf/announcements.175/post-thread` in the debug log: EVERY script on the page throws. Chain, in order: (1) `preamble.min.js` (first script) does a `div=document.createElement(...); parent.appendChild(div); ...; div.parentNode.removeChild(div)` hiddenscroll probe — `div.parentNode` (a real getter, `qjs_el_get_parent_node_data` in [macsurf_qjs.c](browser/netsurf/frontends/macos9/javascript/macsurf_qjs.c)) returns `null` right after the append, so `.removeChild` throws `TypeError: cannot read property 'removeChild' of null`. (2) `jquery-3.5.1.min.js` itself then throws inside Sizzle's `ce()` self-test (`C.createElement`, where `C` is jQuery's captured document reference) — `cannot read property 'createElement' of undefined`, meaning jQuery's own internal document handle is bad at that point too. (3) Because jQuery never finishes initializing, `core-compiled.js` crashes at a **fixed, 100%-reproducible location** (`core-compiled.js:17:415`, confirmed via direct fetch) on `jQuery.support.transition = a` — `jQuery.support` is undefined. (4) Because core-compiled.js dies before registering `XF.Element`, every dependent module — `attachment_manager-compiled.js`, `token_input-compiled.js`, `prefix_menu.min.js`, and **`editor-compiled.js` (Froala) itself** — throws the identical `TypeError: cannot read property 'newHandler' of undefined`. **This is NOT the old 68kmla Froala-size problem** ([project_xenforo_reply_posted.md] / [project_fixes476_forum_editor.md] memory) — that whole mechanism (`s_xf_editor_stub`, the 256KB skip-and-stub) has already been ported into the QuickJS engine (`macsurf_qjs.c` `s_xf_editor_stub`, referencing fixes476/478/479) and is likely moot now: the real 641KB `editor-compiled.js` RUNS (QuickJS handles ES6+ natively, no transpile/skip), it just crashes because `XF.Element` was never set up by the time it runs — one layer downstream of the old bug. **Next step (don't blind-fix, get real data first):** add one targeted log inside `qjs_el_get_parent_node_data`/`qjs_el_append_child_data` printing what dom_document/dom_node the native append actually resolves against for a script-created div appended to `document.body`/`document.documentElement`, to see why `parentNode` reads back `null` immediately after a successful-looking append. Since 68kmla.org is also XenForo (same script stack: preamble → jquery → core-compiled → editor-compiled), this almost certainly blocks its reply box too under the *current* QuickJS tree, even though [project_xenforo_reply_posted.md] recorded a working post under the OLD Duktape engine — that fix predates the QuickJS migration and doesn't carry forward automatically.

- **Text word-merging / awkward wrap ("softwareand", "accessoriesfor") — fixes636 SHIPPED, HARDWARE RESULT: NO IMPROVEMENT, root cause NOT yet correct.** Symptom (real G3, tinkerdifferent.com body text): inter-word spaces collapse so adjacent words merge, and wraps can land mid-word. A 9-agent investigation traced a genuine off-by-one in `macos9_font_split` ([macos9_font.c](browser/netsurf/frontends/macos9/macos9_font.c)): it returned `char_offset = last_space + 1` (the char AFTER the space), but NetSurf core's `layout_text_box_split` ([layout.c:3512](browser/netsurf/content/handlers/html/layout.c#L3512)) does `bool space = (split_box->text[new_length] == ' ')` — it requires the offset to land ON the space itself to detect it and reserve `box->space`. fixes636 changed the return to `last_space` (matching `actual_x`, which already measured width excluding the space). **This was verified against the core contract by reading the exact line and is very likely a real, necessary fix — but the user reports zero visible change on hardware after shipping it**, so either (a) the fix hasn't been hardware-tested with a fresh build yet (confirm the .tar was actually rebuilt/relaunched, not a stale binary — but see DIRECTIVE #1, don't lead with that theory to the user), (b) there is a SECOND independent bug also collapsing the same spaces (upstream of or parallel to `macos9_font_split`, e.g. in how `layout_line` in layout.c consumes the returned offset for THIS fork's variant, or a normalization step earlier that strips spaces before line-breaking ever sees them), or (c) the merged text observed by the user isn't going through the split path at all (fits-on-one-line case, i.e. `macos9_font_measure`/`macos9_run_extent` itself under-counting a space's width in the bulk `TextWidth` path — the workflow's font-measure agent did not find a bug there, but it should be RE-verified specifically for the space character, e.g. does QuickDraw's `TextWidth` on this font/size legitimately return 0 for a bare space in some face/size combination). **NEXT FIX TARGET: add a one-off diagnostic that logs the exact byte offset/width `macos9_font_split` returns for a real overflowing run on this page, and separately log `CharWidth(' ')`/`TextWidth(" ",0,1)` for the active font/size** — do not ship another blind guess; get the number first. Do not confuse this with the webfont/icon-glyph work (fixes630-636), which is a separate, working subsystem.

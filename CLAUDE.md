...
- Last shipped fix: **fixes135b2** — **revert of fixes135b's visual truncation.** fixes135b shipped a redraw-time prefix-fit-and-append-marker but produced corrupted output on hardware: PROBE E2 (180px box) rendered `…PSIS This is a very long line t…` instead of `ELLIPSIS This is a very long li…`, and the same coordinate produced different byte patterns across redraws (`.......x.......h` on subsequent cycles). Root cause not identifiable from code review; likely a stack-buffer-lifetime or measurement-side-effect issue. E5 (8px tiny) rendered correctly as `…`, so the codepath partially worked. fixes135b2 reverts `html_redraw_text_box` back to passing `box->text`/`box->length` through unchanged. The fixes135a libcss plumbing (parser, cascade, computed style, bit slot, dispatch) is preserved. A future round (fixes135c or similar) will reinstate truncation with proper diagnostics first.

...
**Root cause of the whole OS X effort was ONE compile-time flag.** `TARGET_API_MAC_CARBON` was never set for the build: `macsurf_prefix.h` is injected into every TU and includes `<MacTypes.h>` → `ConditionalMacros.h`, which computes the Carbon triplet **then guards itself**; the define lived in `macos9.h:27`, far too late. So `OPAQUE_TOOLBOX_STRUCTS` / `OPAQUE_UPP_TYPES` / `ACCESSOR_CALLS_ARE_FUNCTIONS` were all 0 and **MacSurf compiled as a classic non-Carbon app against Carbon headers for its entire history** — survivable on OS 9 (CarbonLib tolerates it), fatal on OS X. Fixed in fixes951 (prefix, above the MacTypes.h include); mechanical fallout in fixes951c. See [[project_osx_universal_binary]] and [[reference_imac_osx_access]].
...
- **`XMLHttpRequest`/`fetch()` rewritten** in `macsurf_qjs.c` to be genuinely async and real (`fetch()` now returns a real QuickJS `Promise` — `JS_AddIntrinsicPromise` was already wired, just unused for this).
- **`document.createTextNode`/`createDocumentFragment`/`innerHTML=` are now REAL** (previously fake/no-op objects invisible to `qjs_get_node()`, so `appendChild(textNode)` silently no-opped and `innerHTML=` just regex-stripped tags to text). `innerHTML=` uses `dom_hubbub_fragment_parser_create` (zero prior callers anywhere in the codebase).
- **Verified via the S0 ASan harness (new Test 4), which caught two real bugs before hardware ever saw them**: (1) a NULL-`llcache` crash in `content_get_url()` when JS runs against a not-fully-live content — fixed with the same `c->llcache==NULL` guard `macos9_reconvert_host_allowed()` already uses; (2) a **global-buffer-overflow** from reusing the element wrapper (`qjs_wrap_element`, which reads through the element vtable via `dom_element_get_tag_name`) for a DocumentFragment, whose vtable is a different, smaller shape — fixed with a dedicated `qjs_wrap_fragment` that only installs the base-`dom_node_vtable` operations (append/remove/insertBefore/textContent/parentNode) every node subtype actually has. Also found via the harness (a real, non-crashing logic bug): this libdom hubbub binding's fragment parser has **no context-element support** — it silently wraps parsed content in an implied `<html><body>` (same as parsing a full page), so naively appending the fragment's own children put ONE wrapper element into the target instead of the real content; fixed by descending fragment→`<html>`→`<body>` and moving *that* element's children instead.
- **Shipped — awaiting hardware verification on real Facebook** (never self-mark verified per the DIRECTIVE below). Next: S4 (census-driven gap-fill — `IntersectionObserver`, DOM event routing) once a fresh hardware log shows what fires now that data + DOM mutation are both real.
- Ground truth + full technical detail: [[project_facebook_round2]].

*Below is the pre-846 fixes771-834 CSS-coverage summary — unchanged, still accurate for that batch.* `css-coverage` is the **post-2.0 CSS-coverage + fixes development line — 107 commits ahead of `master`** (which sits at the 2.0-release tree + the 3 log-organizer commits). All that work now lives on **`master`** (css-coverage was merged in and deleted 2026-07-17 — see the top banner). **In-app version was bumped to 2.0.5** (fixes827: About box / default UA / `navigator.userAgent` / session banner / Finder `'vers'` all report 2.0.5). 2.0.5 is an **unreleased polish batch** — release-note DRAFT is private at `.private/docs/MacSurf-2.0.5.md` (+ `.txt` paste version); the public CSS support tracker is `docs/css-support.md`. No new tag/GitHub release cut yet (2.0 is still the shipped/latest release).

**Post-2.0 work on `css-coverage` (fixes771→834), verified where noted:**
- **Big CSS coverage push** (the css-coverage thesis): native **`background-clip`** (border/padding/content-box, #255 fixes832), native **`justify-items`** grid alignment (stretch/start/center, #279 fixes833), **`image-rendering`** consume (#256 fixes829), **default font 13px→16px** (#244 fixes830, verified), **inline `style=""` now runs the modern-CSS rewriters** (#277 fixes831, verified — logical props/place-*/grid/calc inline), the **typography cluster** (tab-size/text-align-last/text-justify/hyphens #251, caret-color #252, text-decoration longhands #249), **grid auto-track content sizing** (#62 fixes817-820), logical properties (#247), box-alignment shorthands (#253), rgba-bg compositing (#227). Ground truth: [.private/research/css-gap-inventory-2026-08-14.md](.private/research/css-gap-inventory-2026-08-14.md).
- **Crash fixes:** plot_bitmap **bitmap use-after-free** on refresh (knockout replays a freed content bitmap — routed through the death row, #281 fixes829h, verified) + the temp-mem GWorld reclamation cluster ([[project_gworld_tempmem_reclaim.md]], fixes829c/d/g); **typing latency** = full-viewport CopyBits per keystroke → dirty-rect blit (#239/#212 fixes825, verified, [[project_typing_latency_dirtyrect]]).
- **TLS:** cert validation used LOCAL time as GMT → fresh certs fail on timezone-behind-GMT Macs (#282 fixes834, verified; [[project_tls_localtime_gmt]]).
- **Hacker News** works: login (UA-gated /login, #133 fixes821) + near-Chrome render (border=0 tables 822, SVG-as-image 823), verified. text/plain renders inline (#232/#233).
- **GOTCHA — the `bits[16]` computed-style array is now FULL** (justify-items took the last slot, bits[15] shift 30). The NEXT bit-packed CSS property needs the array **extended** (`bits[17]`) — a structural change to the arena-interning-sensitive `css_computed_style_i`, more delicate than a normal add. See the native-property-add gotcha below.
- **Verification habit:** headless Chrome renders the reference for every `t.html` CSS test LOCALLY ([[reference_headless_chrome]]); only the MacSurf hardware screenshot is the maintainer's.

*Below is the 2.0 RELEASE state (still the shipped/latest release) — unchanged.*

## MacSurf 2.0 RELEASE STATE (2026-07-11)

**MacSurf 2.0 is SHIPPED.** Latest 2.0-line fix was **fixes770** (`29dfd521`, `master`; HEAD `20bfc5f3` adds the log-organizer tooling), pushed to origin + github. **MacSurf 2.0 is SHIPPED** (2026-07-11): tagged **`v2.0`**, the GitHub release is **live and marked latest**, and **`MacSurf2.sit`** is the release asset (also served from `macsurf-web/builds/MacSurf2.sit` and pullable straight from macsurf.org). The `.sit` was **replaced IN PLACE post-launch** with the fixes770 build — same `MacSurf2.sit` asset name → same download URL, **no new tag** (that's the pattern for shipping a post-release build fix). Every version string reports **2.0** (About box / default UA / `navigator.userAgent` / session banner / Finder `'vers'`); release notes ([docs/release-notes/MacSurf-2.0.md](docs/release-notes/MacSurf-2.0.md)), Read Me (`readme.txt`), macintoshgarden blurb (`.private/macintoshgarden-description.html`), README.md, docs/status.md, and both websites (macsurf-web / macsurf-home) are all on 2.0. Predecessor tagged release was **1.68.2 "macQJS"** (`v1.68.2`, `d2eee8e3`); **2.0 absorbed the whole fixes688→765 batch** (incl. the never-shipped 1.68.3 "Blank Page" work), and **post-release fixes767→770** landed the #232 text/plain fix (see the Gotchas entry + [[project_textplain_downloads]]).

**2.0 headline = the blank-screen fix (#207), two real causes both fixed:** (1) hardcoded pointer-ceiling guards (`0x20000000`/`0x28000000`) that reject valid pointers when the app partition maps HIGH on higher-RAM Macs → string interning breaks → no fetcher match → blank ([[project_blank_screen_ptr_ceiling]], fixes716-719; reproduced on maxed-QEMU then fixed via runtime Process-Manager partition bounds); (2) a persisted `deadhosts.txt` condemning the home page across launches ([[project_blank_page_deadhosts]], fixes705, now session-only + self-heals an old poisoned list). Other 2.0 marquee work: cross-signed HTTPS chain reorder (#206, fixes739 — macintoshgarden loads over https, [[project_macintoshgarden_p384_hrr]]); viewport lazy image loading for ALL images (#223, fixes738); URL autocomplete + suggestion dropdown (#231, fixes762-764); text-select cluster (#215/#216/#217/#218, fixes749-752); cache-purge-on-login (#213, fixes750); Apple-menu dedup (#228, fixes753); URL-bar horizontal scroll (#229, fixes756); Get Info `'vers'` (#219, fixes755); typing-latency fix (#212, fixes758-760); UI refresh (fixes720-736); failures-only release logging (fixes765).

**Hardware-verified THIS cycle (G3/G4, OS 9.2.2):** the maintainer confirmed (2026-07-11) the **ENTIRE 2.0 batch is verified** — blank-page fix (#207), macintoshgarden https (#206), lazy images (#223), the text-select cluster (#215/#216/#217/#218), autocomplete + dropdown (#231), cache-on-login (#213), Apple-menu dedup (#228), url-bar scroll (#229), Get Info `'vers'` (#219), typing latency (#212), History/Bookmark managers, UI refresh, and the failures-only log. Nothing in 2.0 is unverified. Workflow: **commit + push immediately** ([[feedback_commit_immediately]]; DIRECTIVE #4 "hold commits" is SUPERSEDED).

**Remotes: origin + github ONLY. Codeberg was DROPPED 2026-07-10** ([[feedback_hold_pushes_until_tested]]). **Working branch is now `master`** (2026-07-17: css-coverage + revamp merged in and deleted everywhere — see the top banner). `password-manager` is the only other branch left, stale/behind — leave it. **Diagnostic logging is now failures-only by default** (fixes765): the crash-only gate (`macsurf_log_is_crash_report` in `macsurf_debug_log.c`) keeps banner + NAV + genuine FAIL/ERROR/exception/crash lines + a **hardcoded whitelist of RECON strings** (`RECON HEAP BOUNDS`, and `RECON MIME` added fixes769). **GOTCHA:** the gate matches those literal strings, NOT a general `RECON` prefix — a NEW `RECON <foo>` diagnostic is silently dropped until you add its string to the whitelist (this cost a round this session). Re-enable the full trace with `MACSURF_NUCLEAR_LOG` in `macsurf_debug_log.c` when chasing a new bug.

**User-uploaded logs auto-organize** (2026-07-11, `tools/log_organizer.py`, stdlib-only, cron `*/2`): the web endpoint drops logs into the `builds/logs/` inbox; the watcher files each into `builds/logs/YYYY/MM/DD/`, regenerates `builds/logs/INDEX.md` (triage table: device, size, NAV/RECON/fail counts, first FAIL line), and posts a **metadata-only** notice (never log contents) to `NOTICES.log` + a Discord webhook (secret in gitignored `builds/logs/.webhook`). **When reading a user log, look in the date folders + `INDEX.md`, not the flat dir.** `builds/logs/` is now **gitignored** (logs can carry sensitive data).
...
- `git fetch --all -q` then, for each of `origin github`: `git log --oneline master..$r/master` (remote ahead of you → divergence, investigate before pushing) and `git log --oneline $r/master..master` (you ahead → unpushed, push it). (Codeberg was dropped 2026-07-10 — only origin + github now.)

Rules:
- Commit each fix as you go (individually revertable). Push to **origin + github** ([[feedback_hold_pushes_until_tested]]; codeberg dropped 2026-07-10). Do NOT hold commits for hardware verification ([[feedback_commit_immediately]]).
...
- **fixes665 — image/font disk cache.** *(Superseded — see the disk-cache entry under Known Gotchas. fixes679 removed image/font caching while #207 was being chased; fixes981-988 rebuilt it on a streaming store.)*
...
- Broader target range: Power Mac G3/G4, Mac OS 9.1-9.2.2. **RAM: 128 MB minimum, 256 MB recommended, 384 MB for the heaviest JS sites** (the old "64 MB" floor is obsolete — it predates the QuickJS + modern-CSS footprint). Partition validated at 224 MB pref / 128 MB min / 16 MB stack (~208 MB heap, stable on github/facebook-class pages); ~76 MB leaner than the prior 307/105/104576 K shipping numbers, all reclaimed from an oversized stack. On a 256 MB / OS 8.1 box the System heap splits free RAM so a 128 MB contiguous block won't fit — drop to 176/64/16 there. See [[project_partition_lean_sizing]].
...
- **`MacSurf.rsrc`** contains the `'carb'` loader marker (ID 0) plus the application's icon family (`ICN#` / `icl4` / `icl8` / `ics#` / `ics4` / `ics8` at ID 128), `FREF` 128 (file type `'APPL'`), `BNDL` 128 (creator `'MPLS'`, mapping icon local-ID 0 → `ICN#` 128 and FREF local-ID 0 → `FREF` 128), `'vers'` 1+2 (Get Info version), and **`'plst'` 0** (fixes936 — the resource-fork `Info.plist` that gives the CFM binary a CFBundle identity under Mac OS X 10.0–10.4 LaunchServices; `'plst'` supersedes `'carb'` on OS X and the two coexist, so one artifact stays correct on both). Pre-compiled binary resource fork generated on Linux by [tools/png_to_mac_icon_rez.py](tools/png_to_mac_icon_rez.py) from the source artwork at [puffpuff.png](puffpuff.png); the script also emits [browser/netsurf/frontends/macos9/MacSurfIcon.r](browser/netsurf/frontends/macos9/MacSurfIcon.r), which [browser/netsurf/frontends/macos9/MacSurf.r](browser/netsurf/frontends/macos9/MacSurf.r) `#include`s so the Rez source matches the binary fork byte-for-byte. CW8 links `.rsrc` files directly into the output resource fork with no Rez step; the `.r` source is the canonical record. Must be listed in the CW8 project alongside the `.c` files. **Creator code is uppercase `'MPLS'`** — Classic Mac type/creator codes are case-sensitive. See [docs/resources.md](docs/resources.md) for the full pipeline.
- **`RegisterAppearanceClient()`** must be called at startup after `InitCursor()`. Matches Classilla's `CBrowserApp` constructor pattern. **It is called unconditionally at [main.c:1730](browser/netsurf/frontends/macos9/main.c#L1730) — there is NO Gestalt gate** (this file claimed one for a long time; `main.c` contains no `Gestalt` call at all). Unconditional is fine on the supported floor (OS 8.6 + CarbonLib 1.6 always has the Appearance Manager), but don't cite a gate that isn't there when debugging Appearance behaviour.
...
The browser fetches directly: HTTP/1.1 over Open Transport (chunked transfer, keep-alive, 3xx redirect follow, connection pooling) and HTTPS over the built-in macTLS stack (TLS 1.3, TLS 1.2 fallback). Origin connections are made straight from the Mac — there is no proxy and no custom protocol. (The legacy Go proxy referenced in older notes is retired; see Components.) The HTTPS fetcher lives in `macos9_tls_fetcher.c` (renamed from `macos9_https_fetcher.c`; tracked, and the old name no longer exists in the tree).
...
- Do not assume JS capability limits (DIRECTIVE #2). The engine is QuickJS/ES2023 and a large browser runtime already runs on-device; missing pieces get filled in-house, not waved off. There is no proxy/offload to fall back on.
...
`WITH_QUICKJS` defaults to `1` ([macsurf_prefix.h:385](browser/netsurf/frontends/macos9/macsurf_prefix.h#L385)) — QuickJS is linked into the base build. (The old `WITHOUT_DUKTAPE` switch is gone along with Duktape itself.) See [JavaScript Engine](#javascript-engine) below.
...
**Files that were once "untracked / renamed in the worktree" — ALL TRACKED AT HEAD as of this writing.** Verified with `git ls-files`; the old names (`macos9_https_fetcher.c`, `browser/libduktape/`, `macsurf_es6.c`, `content/content.c`) no longer exist in the tree at all. They must still be in `MacSurf.mcp`, but there is nothing pending about them:
- `frontends/macos9/macos9_tls_fetcher.c` — the HTTPS fetcher (renamed from `macos9_https_fetcher.c` long ago).
- `frontends/macos9/macos9_content_registry.c` / `.h` — out-of-band live-content registry (anti-UAF, fixes533).
- `frontends/macos9/javascript/macsurf_qjs.c` / `.h` — the QuickJS engine glue.
- `browser/libquickjs/` — the QuickJS engine sources (15 files).
- `content/ns_content.c` — renamed from `content/content.c`.
...
**The engine is QuickJS, not Duktape — committed, tracked, and shipping.** #181 CLOSED. Duktape is not in the tree in any form.

- **QuickJS** is the JS engine, gated by `WITH_QUICKJS` (defaulted to `1` by the `#ifndef` block at [macsurf_prefix.h:384](browser/netsurf/frontends/macos9/macsurf_prefix.h#L384)).
- The engine implementation lives in [browser/netsurf/frontends/macos9/javascript/macsurf_qjs.c](browser/netsurf/frontends/macos9/javascript/macsurf_qjs.c) (tracked). It owns `js_initialise` / `js_newheap` / `js_exec` / `js_fire_event` etc. and runs **ES2023 natively** through `JS_Eval` (`JS_NewRuntime` at heap creation).
- QuickJS sources are at [browser/libquickjs/](browser/libquickjs/) (tracked, 15 files; `cutils.h`, `libregexp.c`, `libunicode.c`, `quickjs.c`, …). Not to be confused with the separate `quickjs-macos9/` standalone port, which the **Linux ASan harness** builds against — see `harness/BUILD-NOTES.md`.
- **The ES6→ES5 transpiler is removed / bypassed (fixes522).** `javascript/macsurf_es6.c` / `.h` are deleted — QuickJS runs modern JS directly, so the transpiler (which was corrupting bundles) is gone. The earlier "JavaScript marathon" hand-built API surface (`macsurf_js.c`, `macsurf_js_dom.c`, the `.bnd` files, etc.) is likewise deleted.
...
- All `browser/libduktape/*` and `content/handlers/javascript/duktape/*` are deleted (not merely untracked — they are gone from the tree).
...
### Current blockers, CSS structural gaps (current ground truth: [.private/research/css-gap-inventory-2026-08-14.md](.private/research/css-gap-inventory-2026-08-14.md); stale reference audits: [.private/CSS_SUPPORT_MATRIX.md](.private/CSS_SUPPORT_MATRIX.md) + [.private/docs/css-status.md](.private/docs/css-status.md))
...
**Other gaps documented in [.private/research/css-gap-inventory-2026-08-14.md](.private/research/css-gap-inventory-2026-08-14.md):**
...
MacSurf handles modern CSS natively in libcss and the layout engine rather than preprocessing it anywhere else. CSS custom properties (`var()`) ship at fixes133-139; full status (per-property coverage, parsed-but-dropped gaps, deferred features) lives in [.private/research/css-gap-inventory-2026-08-14.md](.private/research/css-gap-inventory-2026-08-14.md) — that's the current ground truth, not this file (the older `.private/docs/css-status.md` + `.private/CSS_SUPPORT_MATRIX.md` audits are stale, superseded by the inventory).
...
- **Current release: MacSurf 2.0 — SHIPPED 2026-07-11, tagged `v2.0`, GitHub release live/latest, asset `MacSurf2.sit`.** Release tree was fixes765 (`8cad5551`); the shipped `.sit` was later replaced IN PLACE (same asset name/URL, no new tag) with the **post-release fixes767→770** build (the #232 text/plain fix). Release notes: [docs/release-notes/MacSurf-2.0.md](docs/release-notes/MacSurf-2.0.md). *(Predecessor: MacSurf 1.68 "macQJS", 2026-07-07, tree at fixes674c `9c2d76b7`, below.)*  **1.68 legacy note:** The engine is now macQJS (QuickJS, ES2023) replacing Duktape; on top of the migration this cycle landed the sticky hit-test, login persistence, full text input (caret/selection/Tab/clipboard), image+font disk cache, near-instant startup, and viewport lazy image loading — all G3-verified (see the status block at the top). Release notes: [docs/release-notes/MacSurf-1.68.md](docs/release-notes/MacSurf-1.68.md). The two migration-era crashes (box-walk UAF, cache-hit first-paint) are resolved. [docs/changelog-fixes.md](docs/changelog-fixes.md) has the per-fix history.

**Full fix history (predecessor chain from fixes225 → fixes143a):** see [docs/changelog-fixes.md](docs/changelog-fixes.md).

### Next work queue (priorities from [.private/research/css-gap-inventory-2026-08-14.md](.private/research/css-gap-inventory-2026-08-14.md))

Real-world impact ranked, lowest-effort first within each priority. Numbering reflects the fixes132+ ship order.

[.private/research/css-gap-inventory-2026-08-14.md](.private/research/css-gap-inventory-2026-08-14.md) is the current ground truth for what's shipped vs. open (spec/MDN reference for the gaps: [.private/research/css-missing-properties.md](.private/research/css-missing-properties.md)). **The inventory was re-derived 2026-08-14 against fixes1166** — the numbers below (163 enum slots / 143 of 159 accessors consumed / 16 parsed-but-dropped) reflect the current tree, NOT the 07-13 compile. The list below tracks the active queue only.

Currently-open queue:

- **Highest-value remaining CSS (post fixes834), roughly ranked:**
  - **`min-content`/`max-content`/`fit-content` intrinsic-sizing solver** — STILL the structural prerequisite (no value enums exist; grid `minmax()`/`fit-content()` tracks still collapse to `1fr` in the cssh preprocessor). Unblocks `table-layout:auto` (note: `table-layout: fixed` IS shipped, `layout.c:3063-3087`) and correct flex/grid shrink-to-fit. High effort, unblocks a lot. Lead item.
  - **`justify-self`** (per-item grid override, #279) — still absent; the cssh bridge still shadows it with `text-align` only. **Do NOT extend `bits[16]` — use the fixes1161 row-gap scalar-tail precedent** (self-aligning `int32_t` tail appended at struct end, `autogenerated_computed.h:455-469`) — that is now the proven safe route for a new property.
  - **`background-clip: text`** (gradient headings) — the flashy part of #255 deferred; needs glyph-clip painting. `background-origin` is the cheap companion (same fixes832 plumbing) and is still entirely absent.
  - **Grid Round 2 (#279 remainder):** minmax composition, §12.8 `align-items: stretch` default, fr-row distribution vs definite container height. (placement/span-aware auto sizing largely shipped: fixes817-820 auto tracks, grid-area 4-value + named via cssh, grid-auto-flow column/dense end-to-end.)
- **`#277` inline-rewrite completeness** — fixes831 added the major rewriters to the inline path; the remainder is specifically `grid_placement` (grid-column/grid-row/grid-area longhands) + `grid_template_areas` still skipped inline (cssh_css.c:5352 chain).

- **Grid V2 follow-ups (post fixes150):**
  - **`align-items: stretch` (default behavior)**, cells should stretch to fill the allocated row height. Currently V1 leaves empty space within rows when row track > cell content height (align-items IS read, `layout_grid.c:1393-1405`).
  - **FR row distribution against definite container height**, when the grid container has explicit `height: 500px`, distribute remaining height (after PX rows) across FR rows. Mirrors fixes148's column FR distribution. (Row tracks are px-only V1, `layout_grid.c:440-446`.)
  - **`repeat()`/`minmax()` composition** — `repeat(N, …)` is expanded by cssh, but `minmax()`/`fit-content()`/`calc()` tracks still collapse to `1fr` (cssh_css.c:471-498).
- **Easy wins, independent of everything above:** `caption-side` and `list-style-position` (both still parsed-but-dropped, category (a)), `background-origin` (absent, category (b)). (`outline` is DONE — `redraw.c:4511`; two-value `gap` is DONE — fixes1161; `appearance` is DONE — fixes1052.)

- **Other parsed-but-silently-dropped gaps:** `unicode-bidi`, `writing-mode`, `break-*`, `page-break-*`, `orphans`, `widows`, `fill-opacity`, `stroke-opacity`, `hanging-punctuation`, multi-tier `font-weight` (platform limit, QuickDraw is bold/regular only). (`clip-path`/`mask`/`filter` are NOT in this category — never parsed at all, deferred Group 5.)
- **`transition` / `animation` are NOT in that category — they are never parsed at all.** There is no `CSS_PROP_TRANSITION*` / `CSS_PROP_ANIMATION*` enum anywhere in libcss; the shorthand and every longhand are **deleted from the CSS text by the preprocessor** before libcss sees a byte, via the `DROP_PROPS[]` table in [content/handlers/css/cssh_css.c](browser/netsurf/content/handlers/css/cssh_css.c) (~:3569-3602, fixes191f — they drop together so a stray ident can't trip the parser). Consequence for anyone implementing them: this is a **from-scratch native property add** (the ~15-file template below) plus a timer-driven playback layer, not "wire up a consumer for a value libcss already computed." #34 OPEN.
...
| Host-OS detection (fixes936, OS X tier 1) | `macsurf_osver_init` | `main.c main()` after `macsurf_debug_log_init`, **before** `macsurf_heap_bounds_init` (first consumer) | `MacSurf Debug.log` shows one `RECON OS raw=… ver=9.2.2 osx=0 …` line per launch (`osx=1` on Mac OS X 10.x). On OS X, `RECON HEAP BOUNDS OSX … forced fail-open accept-all` also appears and the partition window is NOT narrowed. **Hardware-unverified on OS X**; fails open to `osx=0` (= today's OS 9 behaviour), so a Gestalt failure can never be a regression. |
...
- **The disk cache: what it actually is, as of fixes988.** `macos9_disk_cache.c`, checked BEFORE the network in both fetchers, one file per URL in `MacSurfData/Cache`, persisting across launches. Do not read the fixes665/fixes679 history as current — it has been rebuilt:
  - **Contents:** HTML/CSS/JS/JSON/XHTML/plain **and images + webfonts** (`MACSURF_CACHE_IMAGES`, fixes988). 1 MB per file, 64 MB directory budget with LRU eviction (`macos9_cache_sweep`).
  - **It carries freshness now (fixes981).** A hit replays the six headers `llcache_fetch_process_header` consumes (Age/Date/ETag/Expires/Cache-Control/Last-Modified), so llcache decides staleness. Before this a hit handed core only a Content-Type, so a cached copy was served **unconditionally and forever** and could never be revalidated — hardware read `reval=14 cond=0`.
  - **A conditional request must never be served from disk** (fixes981). If core sends `If-None-Match`/`If-Modified-Since` it already HAS the body; handing the same stale bytes back answers nothing and asks again next time. Both fetchers skip the disk lookup when those headers are present.
  - **The store STREAMS (fixes987) — never buffer a body to cache it.** The old store accumulated the whole body in RAM by doubling realloc and wrote at FETCH_FINISHED. Measured: the file I/O is free (355 KB in 3 ticks); the BUFFER was not — with images that meant growth up to 727 KB per body on the same 128 MB heap as the decoder's GWorlds, and cold hackaday went 14.7s → 36s (fixes985, reverted in fixes986). Body length is patched into the header LAST, so an aborted/crashed file reads `body_len == 0` and lookup rejects it. Max 16 concurrent streams; a 17th declines to cache.
  - **Result (hardware, fixes988):** a fresh launch onto a visited page serves **65 hits / 4.96 MB from disk, 41 of them images**, and loads in 9.4s vs ~15s. llcache is memory-only (`no_backing_store.c` is NOT_IMPLEMENTED), so this is the only tier that survives a quit.
  - **The 304 path is real now (fixes979).** Both fetchers answer `FETCH_NOTMODIFIED`, so `macos9_capture_extra_headers` no longer strips the conditional headers.

- **Log lines without the `LIFE ` prefix are silently dropped by the failures-only gate, and this has now cost four rounds.** `RECON <foo>` (whitelist is literal strings), the fixes979 `304 not-modified` lines, and both `CACHE store`/`CACHE hit` lines were all written, all invisible, each time looking like the code never ran. **Prefix anything that must survive with `LIFE `.** Corollary that saved a wrong diagnosis: a cumulative counter emitted once per reformat beats a per-event line — it is cheaper AND it is what caught the 304 branch firing while its own log line was being dropped.

- **Log-line gaps are not a profile.** Attributing time to the preceding log line measures where the poll loop was, not what cost time: the gap after a line is mostly the wait for the next fetch. It blamed ~21s on the deathrow drain in BOTH a fast and a slow run of the same page. When something is slow, add a real timer (fixes986 put cumulative ticks inside the cache store and settled in one round what three rounds of log-reading could not).


- **Adding a native libcss property is a ~15-file MECHANICAL add — follow the fixes805/832/833 template exactly, and it's the SAFE path ONLY via the bit-packed `bits[N]` array, never scalar `_i` fields.** The template (background-clip fixes832, justify-items fixes833) touches: `properties.h` (enum value 0x0NN + value enum), `computed.h`/`computed.c` (accessor), `autogenerated_propset.h`/`propget.h` (bit INDEX/SHIFT/MASK), `propstrings.h`/`.c` (property name + any new value strings, kept in enum⇄SMAP-array lockstep), `p_<name>.c` parser + `parse/properties.c` handler table (append at LAST_PROP) + `parse/properties.h` decl, `s_<name>.c` cascade + `select/properties/properties.h` PROPERTY_FUNCS, **BOTH `dispatch.c` AND `s_dispatch.c`** (append the entry, 2nd field = inherited flag 0/1, keep counts equal — a miscount silently corrupts an unrelated property), then the CONSUMER in redraw/layout. New `p_*.c`/`s_*.c` go in `MacSurf.mcp` (flag them, don't edit the .mcp). **The `bits[16]` array is now FULL** (justify-items took bits[15] shift 30, the last slot). **Proven alternatives, both shipped:** (1) a new property that only needs an enum-value can go in a **scalar tail** — self-aligning `int32_t` (or `int32_t` + `css_fixed` pair) appended at the END of `css_computed_style_i`, byte-deterministically written by every cascade/compose/clone/initial path (the fixes1161 row-gap pattern, `autogenerated_computed.h:455-469`; also `appearance` :423-426, `image_rendering` :417-422); (2) a bit-packed property must extend the array to `bits[17]` — a structural `css_computed_style_i` change that the arena interner memcmps, so audit that path first (bit words are deterministic-write and safe; the danger is any SCALAR field mid-struct — see the existing `_i` gotchas below: mid-struct scalar fields are the fixes151b/117 trap; appending at struct end is the safe rule).

- **The cssh_css place/justify BRIDGE must EMIT a property once it becomes real in libcss, and each emitted declaration needs its `; ` terminator.** The bridge ([content/handlers/css/cssh_css.c](browser/netsurf/content/handlers/css/cssh_css.c) `macsurf__rewrite_grid_alignment`) historically SKIPPED emitting `justify-items`/`justify-self` (they were unknown to libcss) and emitted only a `text-align` shadow. When `justify-items` became a native property (fixes833), the bridge still stripped it → the native property never cascaded (probe showed the value stuck at initial). Fix (fixes833b/c): emit the real property AND terminate it with `; ` before the text-align shadow — without the separator the emit fused into `justify-items: centertext-align: center`, which libcss rejects. `justify-self` is still unimplemented → still shadow-only. **When you add a property the cssh preprocessor was approximating, update the bridge to emit the real one.**

- **macTLS validated X.509 cert dates against the Mac's LOCAL time as if it were GMT (fixes834, #282) — fresh certs fail ONLY on timezone-behind-GMT machines.** `OSTLS_GetBearSSLTime` ([macTLS/os9/ostls_time.c](macTLS/os9/ostls_time.c)) read `GetDateTime()` (local) and passed it to `br_x509_minimal_set_time` as GMT. A US-timezone Mac then computes a "now" hours behind real GMT, so a cert issued that same day (notBefore within the offset) reads as not-yet-valid → `br_err=54 (X509_EXPIRED)` at `hs_state=5`. Older certs are unaffected → the maddening "only the fresh cert, only on the imac, macsurf.org loads fine" signature. Fix: apply `ReadLocation` gmtDelta (`GMT = local − gmtDelta`, low 24 bits signed, unsigned arithmetic since mac_seconds > LONG_MAX). **Depends on the Mac's Date&Time Time Zone being set.** Decode any TLS FAIL: `hs_state` = how far the handshake got (5=RecvCertificate); `br_err` = why (54=X509_EXPIRED covers both expired AND not-yet-valid; 55=DN_MISMATCH, see [[project_macintoshgarden_p384_hrr]]). [[project_tls_localtime_gmt]].

- **plot_bitmap GWorlds must be NORMAL memory, never temp — and a bitmap freed mid-redraw is a UAF routed through the death row (fixes829c/d/g/h, #256/#281).** The box-filter downscale reads the source GWorld WHILE allocating the small intermediate; two temp-memory (`useTempMem`=flag 4) GWorlds let the second alloc purge the first's LOCKED pixels → `src[col*4]` fault. Rule: **cached/box-filtered GWorlds = normal memory ONLY (flag 0); the large transient source = normal-primary + temp-fallback** (temp only as a last resort so it can't reclaim a live GWorld; never blank). Separately, a navigation/refresh can free a content's bitmap while a knockout redraw still holds its pointer and replays `plot_bitmap` against the freed struct (`bw`=heap-ptr, `buf`=0x3) — fixed by deferring `macos9_bitmap_destroy` through the existing frontend death row (`macos9_op_depth>0` → defer to the quiescent drain) so the bitmap stays live for the redraw. Full rules + the sanity-bail guard: [[project_gworld_tempmem_reclaim.md]].

- **Typing latency was a full-viewport `CopyBits` per keystroke, not box-count (fixes825, #239/#212).** The GWorld→screen composite in `macos9_handle_update` blitted `off_bounds` (the whole content_rect) every paint, relying on the visRgn to clip — but classic-QD `CopyBits` cost scales with the RECT handed in, not the clipped output. Fix: blit `update_bounds` (the dirty bbox) clamped to content_rect. Speeds EVERY partial redraw (caret/hover/small scroll), not just typing. [[project_typing_latency_dirtyrect]]. **General rule: on classic QD, blit the dirty rect, never a big rect + visRgn clip.**

- **A `text/plain` response DOWNLOADS instead of rendering, because MacSurf has no standalone text/plain content handler** (`textplain_init` is a no-op stub in `misc_stub.c`) — NetSurf's content factory finds no handler and routes it to the downloader. This is what made **Hacker News "download" the login/user pages** (#232): HN rate-limits its dynamic app routes with `429` and serves that body as `text/plain "Sorry."`, so the download was HN's throttle message hitting the no-handler path (verified NOT a Content-Disposition/attachment bug — `cd=0`; and the 429 is HN-side/global — a fresh IP + Chrome UA gets the identical 429). **Stopgap (fixes770): both fetchers relabel `text/plain` → `text/html`** (substituting the forwarded `Content-Type` so the content factory agrees) so plain text renders inline like every browser. Prose/error pages read fine; embedded markup is unescaped until the real handler lands. **Proper fix = #233** (port the real `content/handlers/text/textplain.c`, already in-tree but unwired). Full detail: [[project_textplain_downloads]]. Cache is safe (429 is `status!=200`, never cached; a `200` text/plain is relabeled BEFORE the cache-eligibility check so fresh and cached render identically).

- **The session-banner build timestamp is NOT the build version — never read it as "which build this is," and NEVER conclude a user's build is stale from it (DIRECTIVE #1).** The banner `===  MacSurf 1.68.3   (build <date> <time>)` at [macsurf_debug_log.c:479](browser/netsurf/frontends/macos9/macsurf_debug_log.c#L479) uses `__DATE__`/`__TIME__`, which CW8 freezes to when **that one file** (`macsurf_debug_log.c`) was last compiled. If a fix round doesn't ship `macsurf_debug_log.c`, CW8 doesn't recompile it, so the banner stays frozen at an old time **even though every other shipped file (main.c, window.c, the fetchers, …) IS rebuilt with the current fixes and IS in the binary.** Reading the banner as the build version and telling the user "you're still on the old build, the fix isn't in it" is exactly the DIRECTIVE #1 violation — it wasted a long stretch of this session. To tell which fixes are actually live, look for the fix's OWN log marker (e.g. `mem cache 32MB` from main.c, a new startup line, a menu item), never the banner. If you genuinely want the banner to move, ship `macsurf_debug_log.c` in the tar.

- **The debug log has an aggressive perf-focus FILTER that silently drops most line categories — absence of a log line does NOT mean the code didn't run.** [macsurf_debug_log.c](browser/netsurf/frontends/macos9/macsurf_debug_log.c) lines ~665–793 (fixes253/666/667/669/671/672) suppress whole prefixes unless the matching `MACSURF_VERBOSE_*` macro is defined. Notably line 768 drops **every `https: handshake*` line**, so the fetcher's `https: handshake done host=… resumed=N cipher=…` (TLS-1.3 session-resumption status) NEVER reaches disk on a default build — it looks like resumption is dead when it may be working. Also dropped by default: `https: setup/status/started`, `CACHE`/`https: CACHE`, `hlcache`/`llcache`, `evloop: hb`, `update: bw=`/`redraw_ready`, `gw_event:`, `plot_bitmap`/`img `/`svg_use`, per-element `TBLCOL`/`TBLPICK`/`fixes614 clamp`/`fixes569 LEN`, etc. Consequences: (a) the `[N]` sequence counter shows GAPS where filtered lines fired (normal, not corruption); (b) a long slow fetch looks like a multi-second "black hole" because its in-flight lines are filtered; (c) expected startup/state markers can be absent. **To diagnose fetch / TLS / resumption / cache / redraw behaviour you must build with the relevant `MACSURF_VERBOSE_*` macro** (`MACSURF_VERBOSE_PERF_OFF`, `MACSURF_VERBOSE_FETCH_LOG`, `MACSURF_VERBOSE_REDRAW_LOG`, `MACSURF_VERBOSE_PAINT_LOG`, `MACSURF_VERBOSE_LAYOUT_LOG`) or temporarily delete the specific `strncmp(...) return;` line. `PERFACC` also only sums *instrumented* phase timers — it does NOT count connection-open waits, poll spinning, or inter-fetch gaps, so real wall-clock legitimately exceeds the `total=` it prints (observed: PERFACC `total=10s` on a load the user measured at 20–30s+, with two subresources taking 47s and 135s that landed in no bucket).

- **PERF phase-breakdown (fixes637/638) has a SECOND, deeper bug — retracted, do not trust its numbers yet.** `cascade-done` and `parse-convert-done` are NOT strictly sequential (CSS parses concurrently with HTML, and often finishes BEFORE the html-to-box conversion completes), so subtracting consecutive last-known milestone timestamps under an assumed strict fetch→parse→cascade→reformat→layout→paint order produces nonsense negative deltas whenever real overlap contradicts that assumption (observed: `cascade=-7956062us`). **Next-step fix:** stop computing sequential deltas across phases that can overlap — report the raw absolute per-milestone timestamps (already what the individual `[+Nus] label` lines show) rather than pretending a strict order, or restrict delta math to the subset that's genuinely sequential on the MAIN document's own timeline (fetch→parse-convert-done→layout-done→first-paint-done) and report cascade-done as a separate absolute datum, not folded into the chain.

- **The XenForo reply-editor cascade (see the next entry) ALSO explains the "page looks broken" complaints (blank nav menu, unstyled plain-black-text reply textarea) — it is the SAME root cause, not a separate CSS bug.** Confirmed 2026-07-05: the Abyss theme CSS gates its real nav/editor styling behind a `.has-js` class on `<html>` (`.has-js .p-nav`, `.has-js textarea.input[rows...]`, etc. — a standard progressive-enhancement pattern), which is only added by `preamble.min.js`'s `XF.Feature` swap (`className.replace(/has-no-js/,'has-js')`). Since that script throws (see next entry) before ever reaching that line, `<html>` stays `has-no-js` forever, so the nav renders with none of its layout rules (blank) and the editor textarea falls back to the bare browser-default look (small black text, no chrome). **Do not chase this as a CSS bug** — fixing the `parentNode`/`appendChild` DOM-shim issue below is very likely to fix the visual complaints too, for free, since `has-js` would then get set like it does in every real browser.

- **XenForo reply/editor page (tinkerdifferent.com) fully non-functional — traced to a single upstream DOM-shim cascade, NOT a Froala size issue.** (2026-07-05. **⚠ This entry predates the fixes846 DOM-mutation work and the fixes878 real-libdom traversal work, both of which touched exactly the `parentNode`/`appendChild`/`createElement` surface it blames. RE-TEST before using any of it as a starting point** — the specific `parentNode`-returns-null claim below may simply no longer be true.) Confirmed via `js/xf/announcements.175/post-thread` in the debug log: EVERY script on the page throws. Chain, in order: (1) `preamble.min.js` (first script) does a `div=document.createElement(...); parent.appendChild(div); ...; div.parentNode.removeChild(div)` hiddenscroll probe — `div.parentNode` (a real getter, `qjs_el_get_parent_node_data` in [macsurf_qjs.c](browser/netsurf/frontends/macos9/javascript/macsurf_qjs.c)) returns `null` right after the append, so `.removeChild` throws `TypeError: cannot read property 'removeChild' of null`. (2) `jquery-3.5.1.min.js` itself then throws inside Sizzle's `ce()` self-test (`C.createElement`, where `C` is jQuery's captured document reference) — `cannot read property 'createElement' of undefined`, meaning jQuery's own internal document handle is bad at that point too. (3) Because jQuery never finishes initializing, `core-compiled.js` crashes at a **fixed, 100%-reproducible location** (`core-compiled.js:17:415`, confirmed via direct fetch) on `jQuery.support.transition = a` — `jQuery.support` is undefined. (4) Because core-compiled.js dies before registering `XF.Element`, every dependent module — `attachment_manager-compiled.js`, `token_input-compiled.js`, `prefix_menu.min.js`, and **`editor-compiled.js` (Froala) itself** — throws the identical `TypeError: cannot read property 'newHandler' of undefined`. **This is NOT the old 68kmla Froala-size problem** ([project_xenforo_reply_posted.md] / [project_fixes476_forum_editor.md] memory) — that whole mechanism (`s_xf_editor_stub`, the 256KB skip-and-stub) has already been ported into the QuickJS engine (`macsurf_qjs.c` `s_xf_editor_stub`, referencing fixes476/478/479) and is likely moot now: the real 641KB `editor-compiled.js` RUNS (QuickJS handles ES6+ natively, no transpile/skip), it just crashes because `XF.Element` was never set up by the time it runs — one layer downstream of the old bug. **Next step (don't blind-fix, get real data first):** add one targeted log inside `qjs_el_get_parent_node_data`/`qjs_el_append_child_data` printing what dom_document/dom_node the native append actually resolves against for a script-created div appended to `document.body`/`document.documentElement`, to see why `parentNode` reads back `null` immediately after a successful-looking append. Since 68kmla.org is also XenForo (same script stack: preamble → jquery → core-compiled → editor-compiled), this almost certainly blocks its reply box too under the *current* QuickJS tree, even though [project_xenforo_reply_posted.md] recorded a working post under the OLD Duktape engine — that fix predates the QuickJS migration and doesn't carry forward automatically.

- **Text word-merging / awkward wrap ("softwareand", "accessoriesfor") — FIXED at fixes787-790, #234 CLOSED.** Symptom (real G3, tinkerdifferent.com / 68kmla body text): inter-word spaces collapse so adjacent words merge, and wraps land mid-word. Three rounds, two real causes, and the sequence is the lesson:
  - **fixes636 (the off-by-one) was a real and necessary fix but was NOT the bug the user was seeing.** `macos9_font_split` ([macos9_font.c](browser/netsurf/frontends/macos9/macos9_font.c)) returned `char_offset = last_space + 1` (the char AFTER the space) while NetSurf core's `layout_text_box_split` ([layout.c](browser/netsurf/content/handlers/html/layout.c)) does `bool space = (split_box->text[new_length] == ' ')` — it needs the offset to land ON the space to reserve `box->space`. Correct, shipped, and produced **zero visible hardware change**, which is exactly what sent the next round looking for a diagnostic instead of another guess.
  - **fixes787 got the number.** A `WORK` probe logging the actual split decision + measured space width for real overflowing runs. That is what settled it — see [[project_space_drop_and_perf]] and the "don't ship another blind guess" rule below.
  - **fixes788 — the real mid-word wrap cause.** On reflow, core fills a line and then calls split with a tiny *remaining* width (observed x=30 on "remain surprisingly…"); the else branch returned a mid-word `fit_offset`, so core placed `remai` on the line and `n` on the next. Core expects **`split == 0` = "cannot split here"**: it then force-fits the word if it is first on the line, or char-breaks under `overflow-wrap`, or wraps the whole word. Returning 0 removed the fixes136a-era aggressive mid-word char-break and restored standard behaviour.
  - **fixes789 — gate the core char-break to `inline_count == 1`.** The full 68kmla split log showed body text splitting fine at spaces; the mid-word breaks were no-space tokens (usernames, URLs) jammed into tiny remaining widths (x=11/24/30) and char-broken by the core break-word path (`layout.c`, fixes202). Gating it to first-box-on-line means a genuinely over-long token still breaks, while a partial-line token wraps whole.
  - **fixes790 — verified and probes stripped.** Log-verified 39× `apply=0` (partial-line tokens wrap whole) + 1× `apply=1` (a real over-long first-box token still breaks). #234 closed 2026-07-13.
  - **The transferable lesson (still live):** a fix that is *provably correct against the core contract* can still not be the reported bug. Two rounds were spent theorising about a second space-collapsing bug; one round of instrumentation logging the real split offsets ended it. **Get the number before shipping the next guess.** Do not confuse any of this with the webfont/icon-glyph work (fixes630-636), a separate and working subsystem.

...
- Last shipped fix: **fixes136a** — **`word-break` + `overflow-wrap` libcss plumbing.** Two new properties: `CSS_PROP_WORD_BREAK` (0x088) and `CSS_PROP_OVERFLOW_WRAP` (0x089). Legacy `word-wrap` parses as an alias for `overflow-wrap` — same parser registered twice in `property_handlers`, emits the same bytecode. New files: `p_word_break.c`, `p_overflow_wrap.c`, `s_word_break.c`, `s_overflow_wrap.c`. Bit slots at word 15 shift 7 mask 0x180 (word_break, 2 bits) and shift 9 mask 0x600 (overflow_wrap, 2 bits). Both properties are inherited (=1 in dispatch table). Public accessors `css_computed_word_break()` / `css_computed_overflow_wrap()` in [computed.c](browser/libcss/src/select/computed.c). Test probes W1-W7 in `tests/css/word_break.html` and at `mactrove.com/advanced.html`. Phase A only — layout integration arrives in fixes136b.

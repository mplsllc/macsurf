# ⚠️ READ THIS FIRST: 1.68.2 "macQJS" RELEASE STATE (2026-07-09)

**The working tree is CLEAN and FULLY COMMITTED — trust it.** Tip is **fixes701** (`master`), pushed to all three remotes (origin/github/codeberg). Shipped/tagged release is still **MacSurf 1.68.2 "macQJS"** (GitHub release `v1.68.2`, tag on `d2eee8e3`, asset `MacSurf1682.sit`). Since 1.68.2, commits fixes688→701 are on master but **NOT yet hardware-verified** — perf + TLS + chrome work whose issues stay OPEN pending a G3 pass (see the verify queue below). There are **no untracked build files and no pending renames** — everything in the CW8 build is tracked in git. `TwoPointOh` is a deliberate fallback branch, currently behind master; leave it until an explicit merge. (When editing this banner, re-run the state check — see [Working-tree hygiene](#working-tree-hygiene) — so it never goes stale again.)

**Post-1.68.2 verify queue (code committed, awaiting G3):**
- **fixes688–694** (earlier batch): #208 reflow storm (fixes689 — user's log shows reflows now 2–4, looks fixed), #209 JS-interrupt throttle + #210 querySelector (fixes690), #153 dead-proxy removal (fixes692), #47 History menu / #50 bookmark model (fixes693/694). fixes695 = libdom `.h`→`_i.h` rename; fixes696 = tracked `time.h`+`macos9_desktop_stubs.c`.
- **fixes697** — restore `=== MacSurf session ===` banner through the crash-only log gate (new-session marker).
- **fixes698/699 (#47)** — REAL History manager: own persistent clearable store (urldb was session-only + unclearable) + a day-grouped scrollable manager window (Show All History, Cmd-H).
- **fixes700 (#50)** — REAL Bookmark manager window: folder tree, New Folder / Rename / Delete / Move / Go (Manage Bookmarks, Cmd-B).
- **fixes701 (#206)** — macTLS handshake DIAGNOSTICS (`https: TLS-FAIL ... hs_fail=N`), NOT a fix; fixes691's HRR fix did not work on hardware. Next G3 log's `hs_fail` code pinpoints the P-384 step (6=ECDH math → route NIST curves to `br_ec_prime_i15`). See [[project_macintoshgarden_p384_hrr]].

**Mac side:** no new .c files this batch (managers live in `macos9_chrome_extras.c`), so no MacSurf.mcp change beyond the earlier libdom `_i.h` renames + the two fixes696 files. **#211** (per-node wrapper perf) is the one perf issue still un-coded.

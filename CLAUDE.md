...
**Trust `git status` + the git facts below, not prose.** The active branch is now **`css-coverage`** (NOT master), HEAD is now past fixes834 — **fixes835-846 (#167 Facebook Round 2, Phase 2 "make the JS run") are the newest work**, pushed to origin + github.
...
*Below is the pre-846 fixes771-834 CSS-coverage summary — unchanged, still accurate for that batch.* `css-coverage` is the **post-2.0 CSS-coverage + fixes development line — 107 commits ahead of `master`** (which sits at the 2.0-release tree + the 3 log-organizer commits). All work since 2.0 lives here; treat `css-coverage` as the working branch. **In-app version was bumped to 2.0.5** (fixes827: About box / default UA / `navigator.userAgent` / session banner / Finder `'vers'` all report 2.0.5). 2.0.5 is an **unreleased polish batch** — release-note DRAFT is private at `.private/docs/MacSurf-2.0.5.md` (+ `.txt` paste version); the public CSS support tracker is `docs/css-support.md`. No new tag/GitHub release cut yet (2.0 is still the shipped/latest release).
...
**Remotes: origin + github ONLY. Codeberg was DROPPED 2026-07-10** ([[feedback_hold_pushes_until_tested]]). **Working branch is now `css-coverage`** (the post-2.0 development line, 107 ahead of master — see the top banner); `master` holds the 2.0 release, `password-manager` and `TwoPointOh` are stale/behind — leave them until an explicit merge. **Diagnostic logging is now failures-only by default** (fixes765): the crash-only gate (`macsurf_log_is_crash_report` in `macsurf_debug_log.c`) keeps banner + NAV + genuine FAIL/ERROR/exception/crash lines + a **hardcoded whitelist of RECON strings** (`RECON HEAP BOUNDS`, and `RECON MIME` added fixes769). **GOTCHA:** the gate matches those literal strings, NOT a general `RECON` prefix — a NEW `RECON <foo>` diagnostic is silently dropped until you add its string to the whitelist (this cost a round this session). Re-enable the full trace with `MACSURF_NUCLEAR_LOG` in `macsurf_debug_log.c` when chasing a new bug.

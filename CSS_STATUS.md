...
**fixes135a (2026-05-19): plumbing only.** libcss parses `text-overflow: clip | ellipsis`, the computed style stores the bit correctly, `css_computed_text_overflow()` returns the right enum, invalid values fall back to `clip`. The visual truncation shipped briefly in **fixes135b** but was reverted in **fixes135b2** after hardware testing showed corrupted output (text starting mid-word with non-deterministic byte patterns across redraws). A future round will reinstate visual truncation with proper instrumentation. Until then, `text-overflow: ellipsis` is computed-correct but visually a no-op (text clips at the container edge as before).

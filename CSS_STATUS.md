...
**fixes135a + fixes135b (2026-05-19): partial.** Parser/cascade/computed plumbing landed in 135a; redraw-time truncation landed in 135b. Active when a styled ancestor declares `text-overflow: ellipsis` AND its `overflow-x` is not `visible` AND the text would paint past the active clip rect. Binary-searches for the longest UTF-8 prefix that fits in `available_width - ellipsis_width` and writes `prefix + U+2026` to a 512-byte stack buffer; the macos9 plot_text path folds U+2026 to MacRoman 0xC9 ([macos9_font.c:109](browser/netsurf/frontends/macos9/macos9_font.c#L109)). Falls back to a single dot if the marker can't be measured. Deferred to V2: multi-line ellipsis, two-value form (`text-overflow: clip ellipsis`), custom string marker, RTL/start-side ellipsis, ellipsis across complex nested inline boxes.

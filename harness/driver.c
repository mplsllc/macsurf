...
/* Test 65 — the WOFF2 @font-face reconstructor (macos9_woff2.c + the
 * vendored Brotli decoder). Linked into the harness via GLUE_PLAIN_SRC. */
...
/* Test 65: read a whole file as raw bytes (binary-safe). Returns a malloc'd
 * buffer and its length, or NULL on any failure. */
static unsigned char *t65_read(const char *path, long *len)
{
	FILE *f = fopen(path, "rb");
	long n;
	unsigned char *buf;
...
#define T48_RECONVERT() do { \
	...
	/* --- Test 65: WOFF2 @font-face reconstruction (macos9_woff2.c) --------
	 * The frontend fetches a @font-face src, sees a "wOF2" magic,
	 * Brotli-decompresses + rebuilds a standard sfnt
	 * (macos9_woff2_to_ttf), then parses it exactly like a raw .ttf/.otf
	 * (webfont_parse). This test runs the decoder over the real corpus in
	 * fonts/ and asserts the output is a structurally valid sfnt carrying
	 * the exact tables webfont_parse() requires, plus an exact byte-compare
	 * against libwoff2dec's reference output (fonts/*.ref.ttf).
	 *
	 * Endianness: the Brotli bitstream reader switches on
	 * BROTLI_BIG_ENDIAN / BROTLI_LITTLE_ENDIAN (BROTLI_UNALIGNED_LOAD*LE
	 * byte-swaps on big-endian). This host is little-endian; the Mac is
	 * big-endian PPC, pinned via BROTLI_BUILD_BIG_ENDIAN under __MWERKS__
	 * in macos9_brotli.c. The PPC path is verified separately by
	 * cross-compiling the same TUs with powerpc-linux-gnu-gcc -D__MWERKS__
	 * and byte-comparing under qemu-ppc: 7/7 identical to libwoff2dec.
	 *
	...
	{
		static const char *t65_ok[] = {
			"fonts/roboto.woff2",
			"fonts/lora-v14-latin-regular.woff2",
			"fonts/lora-v14-latin-italic.woff2",
			"fonts/lora-v14-latin-700.woff2",
			"fonts/Metropolis-Regular.woff2",
			"fonts/Metropolis-SemiBold.woff2",
			"fonts/Metropolis-Bold.woff2",
		};
		static const char *t65_bad[] = {
			"fonts/fa-solid-900.woff2",	/* corrupt upstream */
		};
		static const char *t65_req[] = {
			"head", "maxp", "hhea", "hmtx", "cmap", "loca", "glyf",
		};
		unsigned int t65_pass = 0, t65_fail = 0;
		int t65_corpus = 0;
		size_t i;

		{
			long probe = 0;
			unsigned char *p = t65_read("fonts/roboto.woff2", &probe);
			if (p == NULL) {
				fprintf(stderr, "SKIP: fonts/ corpus not present\n");
			} else {
				free(p);
				t65_corpus = 1;
			}
		...
		if (t65_corpus) {
			/* run the real assertions */
			for (i = 0; i < sizeof(t65_ok) / sizeof(t65_ok[0]); i++) {
				unsigned char *src, *mine, *ref;
				...
				src = t65_read(t65_ok[i], &src_len);
				if (src == NULL) {
					fprintf(stderr, "FAIL: Test 65 -- cannot read %s\n",
							t65_ok[i]);
					t65_fail++;
					continue;
				}
				if (!macos9_woff2_to_ttf(src, src_len, &mine, &mine_len)) {
				...
				free(src);
				if (mine == NULL || mine_len < 12) {
					fprintf(stderr, "FAIL: Test 65 -- %s empty output\n",
							t65_ok[i]);
					bad = 1;
				}
				if (!bad) {
				...
				if (!bad) {
					ntab = (unsigned short) ((mine[4] << 8) | mine[5]);
					if (ntab < 1 || ntab > 64) {
						fprintf(stderr,
							"FAIL: Test 65 -- %s bad numTables %u\n",
							t65_ok[i], (unsigned) ntab);
						bad = 1;
					}
				}
				if (!bad) {
					for (j = 0; j < (size_t) ntab; j++) {
						...
						off = ((unsigned int) mine[12 + j * 16 + 8] << 24) |
						      ((unsigned int) mine[12 + j * 16 + 9] << 16) |
						      ((unsigned int) mine[12 + j * 16 + 10] << 8) |
						      (unsigned int) mine[12 + j * 16 + 11];
						len = ((unsigned int) mine[12 + j * 16 + 12] << 24) |
						      ((unsigned int) mine[12 + j * 16 + 13] << 16) |
						      ((unsigned int) mine[12 + j * 16 + 14] << 8) |
						      (unsigned int) mine[12 + j * 16 + 15];
						if (off > (unsigned int) mine_len ||
								off + len > (unsigned int) mine_len) {
							fprintf(stderr,
								"FAIL: Test 65 -- %s table %u out of bounds\n",
								t65_ok[i], (unsigned) j);
							bad = 1;
							break;
						}
						if (memcmp(mine + 12 + j * 16, "head", 4) == 0)
							head_off = off;
					}
				}
				if (!bad) {
				...
				if (!bad) {
					/* head: unitsPerEm 16..16384, magicNumber 0x5F0F3CF5 */
					unsigned int upem, hmagic;

					if (head_off == 0xFFFFFFFFu ||
							head_off + 52 > (unsigned int) mine_len) {
						fprintf(stderr,
							"FAIL: Test 65 -- %s head table missing\n",
							t65_ok[i]);
						bad = 1;
					} else {
						upem = (unsigned int) ((mine[head_off + 18] << 8) |
								mine[head_off + 19]);
						hmagic = ((unsigned int) mine[head_off + 12] << 24) |
						         ((unsigned int) mine[head_off + 13] << 16) |
						         ((unsigned int) mine[head_off + 14] << 8) |
						         (unsigned int) mine[head_off + 15];
						if (upem < 16 || upem > 16384 ||
								hmagic != 0x5F0F3CF5u) {
							fprintf(stderr,
								"FAIL: Test 65 -- %s bad head (upem=%u magic=%08x)\n",
				...
				if (!bad) {
					...
					for (j = 0; j + 4 <= (size_t) mine_len; j += 4) {
						sum += ((unsigned int) mine[j] << 24) |
						       ...
						       ((unsigned int) mine[j + 2] << 8) |
						       (unsigned int) mine[j + 3];
					}
					if (mine_len & 3) {
						unsigned int v = 0;
						for (k = j; k < (size_t) mine_len; k++)
				...
				if (!bad) {
					/* exact byte-compare vs libwoff2dec reference */
					snprintf(ref_path, sizeof(ref_path), "%s.ref.ttf",
							t65_ok[i]);
					ref = t65_read(ref_path, &ref_len);
					if (ref == NULL) {
						fprintf(stderr,
							"FAIL: Test 65 -- %s no reference %s\n",
							t65_ok[i], ref_path);
						bad = 1;
					} else if (mine_len != ref_len ||
							memcmp(mine, ref, (size_t) ref_len) != 0) {
						fprintf(stderr,
							"FAIL: Test 65 -- %s differs from libwoff2dec "
							"(mine=%ld ref=%ld)\n",
							t65_ok[i], mine_len, ref_len);
						bad = 1;
					}
					free(ref);
				}
				if (!bad) {
					fprintf(stderr, "PASS: Test 65 -- %s (%ld bytes, "
						"sfnt=%08x, matches libwoff2dec)\n",
						t65_ok[i], mine_len, magic);
					t65_pass++;
				} else {
					t65_fail++;
				}
		...
		if (t65_fail) {
			fprintf(stderr, "FAIL: Test 65 -- %u pass, %u fail\n",
					t65_pass, t65_fail);
			return 1;
		}

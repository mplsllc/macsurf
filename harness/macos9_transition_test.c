...
bool test_macos9_transition_opacity(void)
{
    ...
    /* 30. scoped retirement and cross-window independence */
    extern bool test_macos9_transition_scoped_retirement(void);
    if (!test_macos9_transition_scoped_retirement()) {
        fprintf(stderr, "FAIL scoped retirement test\n");
        ok = false;
        goto done_op;
    }
...
bool test_macos9_transition_scoped_retirement(void)
{
    dom_node *n1 = NULL;
    dom_node *n2 = NULL;
    struct html_content *c1 = (struct html_content *)0x1000;
    struct dom_document *d1 = (struct dom_document *)0x2000;
    struct html_content *c2 = (struct html_content *)0x3000;
    struct dom_document *d2 = (struct dom_document *)0x4000;
    css_fixed out = 0;
    css_transition_timing_entry timing;
    bool ok = true;
    uint32_t now = 100;
    ...
    n2 = create_test_node();
    if (n1 == NULL || n2 == NULL) {
        fprintf(stderr, "FAIL scoped_retirement create test nodes\n");
        ok = false;
        goto done;
    }

    macsurf_transition_test_now = now;
    if (macsurf_transition_create(n1, CSS_PROP_OPACITY, 0, 1024, 0, 1024, timing, now, n1) == 0) {
        fprintf(stderr, "FAIL scoped_retirement create n1 effect\n");
        ok = false;
        goto done;
    }
    if (macsurf_transition_create(n2, CSS_PROP_OPACITY, 0, 1024, 0, 1024, timing, now, n2) == 0) {
        fprintf(stderr, "FAIL scoped_retirement create n2 effect\n");
        ok = false;
        goto done;
    }
    macsurf_transition_set_owner(n1, CSS_PROP_OPACITY, c1, d1);
    macsurf_transition_set_owner(n2, CSS_PROP_OPACITY, c2, d2);

    if (macsurf_transition_active_count() != 2) {
        fprintf(stderr, "FAIL scoped_retirement active count != 2 (got %d)\n",
            macsurf_transition_active_count());
        ok = false;
        goto done;
    }

    /* Advance to midpoint (30 ticks) */
    now += 30;
    macsurf_transition_test_now = now;
    if (!macsurf_transition_get_presented(n1, CSS_PROP_OPACITY, &out, now) || out < 400 || out > 600) {
        fprintf(stderr, "FAIL scoped_retirement n1 midpoint presented %d\n", (int)out);
        ok = false;
        goto done;
    }
    if (!macsurf_transition_get_presented(n2, CSS_PROP_OPACITY, &out, now) || out < 400 || out > 600) {
        fprintf(stderr, "FAIL scoped_retirement n2 midpoint presented %d\n", (int)out);
        ok = false;
        goto done;
    }

    /* Full reconvert of c1 / d1 retires only c1 / d1 effects */
    macsurf_transition_retire_content(c1);
    macsurf_transition_retire_document(d1);

    /* n1 effect must be gone (renderer falls back to computed style B) */
    if (macsurf_transition_get_presented(n1, CSS_PROP_OPACITY, &out, now)) {
        fprintf(stderr, "FAIL scoped_retirement n1 effect still active after c1 retirement\n");
        ok = false;
        goto done;
    }

    /* n2 effect must remain active and animating (cross-window independence) */
    if (!macsurf_transition_get_presented(n2, CSS_PROP_OPACITY, &out, now)) {
        fprintf(stderr, "FAIL scoped_retirement n2 effect lost when c1 retired\n");
        ok = false;
        goto done;
    }
    if (macsurf_transition_active_count() != 1) {
        fprintf(stderr, "FAIL scoped_retirement active count != 1 after c1 retirement (got %d)\n",
            macsurf_transition_active_count());
        ok = false;
        goto done;
    }

    /* Subsequent ticks must NOT overwrite Style B for n1 */
    now += 15;
    macsurf_transition_test_now = now;
    macsurf_transition_tick(NULL);

    if (macsurf_transition_get_presented(n1, CSS_PROP_OPACITY, &out, now)) {
        fprintf(stderr, "FAIL scoped_retirement tick recreated or overwrote n1 effect\n");
        ok = false;
        goto done;
    }
    /* n2 continues animating */
    if (!macsurf_transition_get_presented(n2, CSS_PROP_OPACITY, &out, now) || out <= 600) {
        fprintf(stderr, "FAIL scoped_retirement n2 did not advance (out=%d)\n", (int)out);
        ok = false;
        goto done;
    }

    /* Retire c2 / d2 */
    macsurf_transition_retire_content(c2);
    if (macsurf_transition_has_active()) {
        fprintf(stderr, "FAIL scoped_retirement effects remain after c2 retirement\n");
        ok = false;
        goto done;
    }
    if (macsurf_transition_scheduler_active()) {
        fprintf(stderr, "FAIL scoped_retirement scheduler active after all retired\n");
        ok = false;
        goto done;
    }

    fprintf(stderr, "  [2B-2] scoped retirement & cross-window independence PASS\n");

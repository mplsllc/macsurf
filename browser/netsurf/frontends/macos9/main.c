...
static void macos9_handle_menu(short menu_id, short item) {
	...
	case MENU_EDIT:
	        front = FrontWindow();
	        gw = front ? macos9_find_window(front) : NULL;
	        if (gw && gw->url_field_active && gw->url_te) {
	                switch (item) {
	                case ITEM_EDIT_CUT:   TECut(gw->url_te); break;
	                case ITEM_EDIT_COPY:  TECopy(gw->url_te); break;
	                case ITEM_EDIT_PASTE: TEPaste(gw->url_te); break;
	                default: break;
	                }
...
#define kInitOTForApplicationMask 0x00000002
	...
	netsurf_init(NULL);	{ extern nserror browser_set_dpi(int dpi); browser_set_dpi(96); }

 ...
 * at fixes218 so the HTTPS fetcher hits the same store. cache_lookup /
 * cache_store are now public via macos9_cache_*. The per-fetch capture
 * buffer + overflow latch stay here in macos9_fetch_ctx (cache_capture

...
/* ===================================================================== */
/* STAGE 5: for-of -> indexed for loop                                   */
/* ===================================================================== */

/* Append unsigned v as decimal to out[*o]. */
static int
es6_put_uint(char *out, size_t *o, size_t cap, unsigned long v)
{
	char tmp[16];
	int k = 0;
	if (v == 0) tmp[k++] = '0';
	while (v > 0) { tmp[k++] = (char)('0' + (int)(v % 10)); v /= 10; }
	while (k > 0) { if (*o + 1 >= cap) return 0; out[(*o)++] = tmp[--k]; }
	return 1;
}

/* Scan a single statement from s[b]; return index just past its terminating
 * ';' (at depth 0), or the enclosing '}' / end. Lexer-aware. */
static size_t
es6_stmt_end(const char *s, size_t n, size_t b)
{
	size_t i = b;
	int st = ES_CODE;
	int prev = 0;
	...
	while (i < n) {
		...
		if (st == ES_LCOM) { if (c == '\n') st = ES_CODE; i++; continue; }
		if (st == ES_BCOM) { if (c == '*' && i + 1 < n && s[i + 1] == '/') { i += 2; st = ES_CODE; } else i++; continue; }
		if (c == '\\' && i + 1 < n) i += 2; else { if (c == '/') st = ES_CODE; i++; }
	}
	return n;
}

/* Find the first for-of in s[0..n). Distinguishes from for(;;) (has ';' at
 * depth 1) and for-in. Fills the loop-var region [lhs0,lhs1), iterable region
 * [rhs0,rhs1), body [b0,bend), blk=1 if body is a { } block. Returns 1/0. */
static int
es6_find_forof(const char *s, size_t n, size_t *f,
	size_t *lhs0, size_t *lhs1, size_t *rhs0, size_t *rhs1,
	size_t *b0, size_t *bend, int *blk)
{
	size_t i;
	int st;
	int prev_sig;
	i = 0; st = ES_CODE; prev_sig = 0;
	while (i < n) {
		char c = s[i];
		if (st == ES_CODE) {
			if (c == '/' && i + 1 < n && s[i + 1] == '/') { i += 2; st = ES_LCOM; continue; }
			...
			if (es6_is_ident((unsigned char) c) &&
				...
				while (i < n && es6_is_ident((unsigned char) s[i])) i++;
				ide = i;
				if (ide - ids == 3 && strncmp(s + ids, "for", 3) == 0) {
					size_t j = ide;
					while (j < n && (s[j] == ' ' || s[j] == '\t' || s[j] == '\n' || s[j] == '\r')) j++;
					if (j < n && s[j] == '(') {
						size_t p0 = j, k = j + 1, ofs = 0, ofe = 0, p1 = 0;
						int depth = 1, st2 = ES_CODE, prev2 = '(', bad = 0;
						while (k < n && depth > 0) {
							char d = s[k];
							if (st2 == ES_CODE) {
								if (d == '/' && k + 1 < n && s[k + 1] == '/') { k += 2; st2 = ES_LCOM; continue; }
								if (d == '/' && k + 1 < n && s[k + 1] == '*') { k += 2; st2 = ES_BCOM; continue; }
								if (d == '/' && es6_regex_ctx(prev2)) { k++; st2 = ES_RE; continue; }
								if (d == '\'') { k++; st2 = ES_SQ; prev2 = d; continue; }
								if (d == '"') { k++; st2 = ES_DQ; prev2 = d; continue; }
								if (d == '`') { k++; st2 = ES_TMPL; prev2 = d; continue; }
								if (es6_is_ident((unsigned char) d) && (k == 0 || !es6_is_ident((unsigned char) s[k - 1]))) {
									size_t ws = k, we;
									while (k < n && es6_is_ident((unsigned char) s[k])) k++;
									we = k;
									if (ofs == 0 && we - ws == 2 && strncmp(s + ws, "of", 2) == 0 && depth == 1) { ofs = ws; ofe = we; }
									else if (ofs == 0 && we - ws == 2 && strncmp(s + ws, "in", 2) == 0 && depth == 1) { bad = 1; break; }
									prev2 = s[we - 1];
									continue;
								}
								if (d == '(' || d == '[' || d == '{') depth++;
								else if (d == ')' || d == ']' || d == '}') { depth--; if (depth == 0) { p1 = k; break; } }
								else if (d == ';' && depth == 1) { bad = 1; break; }
								if (d != ' ' && d != '\t' && d != '\n' && d != '\r') prev2 = d;
								k++;
								continue;
							}
							if (st2 == ES_SQ) { if (d == '\\' && k + 1 < n) k += 2; else { if (d == '\'') st2 = ES_CODE; k++; } continue; }
							if (st2 == ES_DQ) { if (d == '\\' && k + 1 < n) k += 2; else { if (d == '"') st2 = ES_CODE; k++; } continue; }
							if (st2 == ES_TMPL) { if (d == '\\' && k + 1 < n) k += 2; else { if (d == '`') st2 = ES_CODE; k++; } continue; }
							if (st2 == ES_LCOM) { if (d == '\n') st2 = ES_CODE; k++; continue; }
							if (st2 == ES_BCOM) { if (d == '*' && k + 1 < n && s[k + 1] == '/') { k += 2; st2 = ES_CODE; } else k++; continue; }
							if (d == '\\' && k + 1 < n) k += 2; else { if (d == '/') st2 = ES_CODE; k++; }
						}
						if (!bad && ofs > p0 && p1 > ofe && ofe < p1) {
							size_t b = p1 + 1;
							while (b < n && (s[b] == ' ' || s[b] == '\t' || s[b] == '\n' || s[b] == '\r')) b++;
							if (b < n && s[b] == '{') {
								size_t be = es6_block_end(s, n, b);
								if (be > 0) { *f = ids; *lhs0 = p0 + 1; *lhs1 = ofs; *rhs0 = ofe; *rhs1 = p1; *b0 = b; *bend = be; *blk = 1; return 1; }
							} else if (b < n) {
								size_t be = es6_stmt_end(s, n, b);
								if (be > b) { *f = ids; *lhs0 = p0 + 1; *lhs1 = ofs; *rhs0 = ofe; *rhs1 = p1; *b0 = b; *bend = be; *blk = 0; return 1; }
							}
						}
					}
				}
...
/* Iteratively rewrite for-of loops to indexed for loops:
 *   for (LHS of RHS) BODY  ->
 *   for(var _oN$=(RHS),_iN$=0;_iN$<_oN$.length;_iN$++){LHS=_oN$[_iN$]; BODY}
 * Array-indexed (the common case); a later stage can add the iterator
 * protocol for Map/Set. Returns final length, 0 on overflow/OOM. */
static size_t
es6_forof_pass(const char *src, size_t len, char *out, size_t cap)
{
	char *a, *b, *tmp;
	size_t alen, wmax, gen;
	wmax = len * 4 + 4096;
	if (len + 1 > wmax) { if (len < cap) { memcpy(out, src, len); out[len] = '\0'; return len; } return 0; }
	a = (char *)malloc(wmax);
	b = (char *)malloc(wmax);
	if (a == NULL || b == NULL) { free(a); free(b); if (len < cap) { memcpy(out, src, len); out[len] = '\0'; return len; } return 0; }
	memcpy(a, src, len); a[len] = '\0'; alen = len;
	gen = 0;
	for (;;) {
		size_t f, lhs0, lhs1, rhs0, rhs1, b0, bend, o;
		int blk, ok = 1;
		if (gen > 100000UL) break;
		if (!es6_find_forof(a, alen, &f, &lhs0, &lhs1, &rhs0, &rhs1, &b0, &bend, &blk)) break;
		o = 0;
#define FO_PUTR(p, e) do { size_t _i = (p); while (_i < (e)) { if (o + 1 >= wmax) { ok = 0; break; } b[o++] = a[_i++]; } } while (0)
#define FO_PUTS(str) do { const char *_p = (str); while (*_p) { if (o + 1 >= wmax) { ok = 0; break; } b[o++] = *_p++; } } while (0)
#define FO_PUTN()    do { if (!es6_put_uint(b, &o, wmax, (unsigned long)gen)) ok = 0; } while (0)
		FO_PUTR(0, f);
		FO_PUTS("for(var _o"); FO_PUTN(); FO_PUTS("$=(");
		FO_PUTR(rhs0, rhs1);
		FO_PUTS("),_i"); FO_PUTN(); FO_PUTS("$=0;_i"); FO_PUTN();
		FO_PUTS("$<_o"); FO_PUTN(); FO_PUTS("$.length;_i"); FO_PUTN(); FO_PUTS("$++){");
		FO_PUTR(lhs0, lhs1);
		FO_PUTS("=_o"); FO_PUTN(); FO_PUTS("$[_i"); FO_PUTN(); FO_PUTS("$];");
		if (blk) { FO_PUTR(b0 + 1, bend - 1); }
		else { FO_PUTR(b0, bend); }
		FO_PUTS("}");
		FO_PUTR(bend, alen);
#undef FO_PUTR
#undef FO_PUTS
#undef FO_PUTN
		...
		if (o + 1 >= wmax) break;
		b[o] = '\0';
		tmp = a; a = b; b = tmp; alen = o; gen++;
	}
	if (alen < cap) { memcpy(out, a, alen); out[alen] = '\0'; free(a); free(b); return alen; }
...
macsurf_es6_transpile(const char *src, size_t len, char *out, size_t cap)
{
	...
	/* Pass 6: for-of -> indexed for loop. */
	dst = (cur == buf1) ? buf2 : buf1;
	m = es6_forof_pass(cur, n, dst, wmax);
	if (m != 0) { cur = dst; n = m; }

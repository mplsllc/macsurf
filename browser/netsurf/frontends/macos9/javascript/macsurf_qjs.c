 ...
 * Node.prototype via qjs_el_install_proto_surface (fixesXXXX, #211), so they
...
static void qjs_el_install_js_helpers(JSContext *ctx, JSValue proto)
{
	static const char *src =
		...
		/* fixes878 — the node-oriented traversal surface (cloneNode/
		 * contains/firstChild/lastChild/nextSibling/previousSibling/
		 * childNodes/hasChildNodes) is NO LONGER defined here. It moved to
		 ...
		 * zero per-element closures (fixesXXXX #211). */
		"})";
	JSValue fn;
...
static JSValue qjs_wrap_text_node(JSContext *ctx, dom_text *tn)
{
	...
	/* fixesXXXX (#211) — nodeValue/data/textContent live on
...
static JSValue qjs_wrap_fragment(JSContext *ctx, dom_document_fragment *frag)
{
	...
	/* fixesXXXX (#211) — parentNode + the node-level traversal surface are on

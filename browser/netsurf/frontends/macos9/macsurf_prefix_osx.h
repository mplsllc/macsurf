/*
 * MacSurf X prefix header - PowerPC Mach-O / Mac OS X
 */

#ifndef MACSURF_PREFIXX_H
#define MACSURF_PREFIXX_H

/* ============================================================
 * MacSurf X target identity.
 *
 * This prefix belongs to the native PowerPC Mach-O Mac OS X target.
 * Keep these project-owned identities separate from compiler/linker
 * predefined macros so shared source can distinguish MacSurf targets
 * explicitly and predictably.
 */
#define MACSURF_CLASSIC 0
#define MACSURF_OSX 1
#define MACSURF_MACHO 1
#define MACSURF_CFM 0
...
/* MacSurf X is still a Carbon frontend at this stage; Mach-O changes the
 * binary/runtime model, not the Carbon API contract. */
 ...
 *
 * The current MacSurf frontend and NetSurf compatibility layer use
 * __MACOS9__ as a frontend/shim selector, not merely as an OS identity.
 * It gates Carbon/Toolbox code, config.h feature choices, inet shims, TEC,
 * and other paths still shared by the first MacSurf X build.  Keep it
 * defined until those call sites are migrated to MACSURF_CLASSIC /
 * MACSURF_OSX or to platform-neutral feature tests.
 *
 * Do NOT use __MACOS9__ for new MacSurf X code.  New target-specific code
 * should test MACSURF_OSX (or MACSURF_CLASSIC) instead.
 */
#define __MACOS9__ 1

		...
		const struct redraw_context *ctx)
{
	...
	* for BOX_TEXT it's in an inline */
	...
	if (box->style && box->type == BOX_INLINE && box->inline_end &&
		 ...
		 * first and the RIGHT only on the last (redraw_border.c:771,777).
		 * That is precisely `slice`, the initial value.
		 *
		 * `clone` means every fragment is decorated as a complete box, so
		 * forcing both flags true on every fragment IS the clone rendering
		 * -- background and all four borders per fragment. The property has
		 * had a computed-style field since it was parsed
		 * (autogenerated_computed.h:404) but no consumer, so it silently did
		 * nothing. */
		bool bdb_clone = (box->style != NULL &&
			css_computed_box_decoration_break(box->style) ==
				CSS_BOX_DECORATION_BREAK_CLONE);
		int ib_x;
		int ib_y = y;
		int ib_p_width;
		...
		if (ctx->plot->clip(ctx, &r) != NSERROR_OK)
			return false;
		if (!html_redraw_inline_borders(box, b, &r, scale,
				first || bdb_clone, true, ctx))
			return false;

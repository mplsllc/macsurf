		...
		css_style *result)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) &&
		...
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_MIN_WIDTH,
				0, MIN_WIDTH_FIT_CONTENT);

	} else if ((token->type == CSS_TOKEN_FUNCTION) && (css__calc_function(c, token) != CSS_CALC_FUNC_NONE)) {
		error = css__parse_calc(c, vector, ctx, result, buildOPV(CSS_PROP_MIN_WIDTH, 0, MIN_WIDTH_CALC), UNIT_PX, css__calc_function(c, token));
	} else {
		css_fixed length = 0;
		uint32_t unit = 0;
		*ctx = orig_ctx;

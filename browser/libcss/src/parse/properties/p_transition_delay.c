		...
		css_style *result)
{
	...
	while (token != NULL) {
		...
		error = parse_single_time(c, token, &del);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}

		/* Negative delays are explicitly VALID in CSS Transitions */
		delays[count++] = del;
		parserutils_vector_iterate(vector, ctx);
	...
	for (i = 0; i < count; i++) {
		error = css__stylesheet_style_append(result, (css_code_t)delays[i]);
		if (error != CSS_OK) { *ctx = orig_ctx; return error; }
	}

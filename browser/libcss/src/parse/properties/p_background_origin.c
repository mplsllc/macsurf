 ...
 *
 * Parse background-origin's box-model positioning area.
 */
		...
		css_style *result)
{
	...
	if (lwc_string_caseless_isequal(token->idata, c->strings[INHERIT],
		...
		error = css_stylesheet_style_initial(result,
			...
			c->strings[REVERT], &match) == lwc_error_ok && match) {
		...
		error = css_stylesheet_style_unset(result,
			...
			c->strings[PADDING_BOX], &match) == lwc_error_ok && match) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BACKGROUND_ORIGIN, 0,
				CSS_BACKGROUND_ORIGIN_PADDING_BOX);
	} else if (lwc_string_caseless_isequal(token->idata,
			c->strings[BORDER_BOX], &match) == lwc_error_ok && match) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BACKGROUND_ORIGIN, 0,
				CSS_BACKGROUND_ORIGIN_BORDER_BOX);
	} else if (lwc_string_caseless_isequal(token->idata,
			c->strings[CONTENT_BOX], &match) == lwc_error_ok && match) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BACKGROUND_ORIGIN, 0,

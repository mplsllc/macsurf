		...
		css_style *result)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) &&
		...
		error = css__stylesheet_style_appendOPV(result,
			...
			&match) == lwc_error_ok && match)) {
		/* intrinsic-sizing round 1 (#0 in the css-gap-inventory
		 * backlog): min-content/max-content/fit-content keywords. */
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_WIDTH,
				0, WIDTH_MIN_CONTENT);
		...
		error = css__parse_calc(c, vector, ctx, result, buildOPV(CSS_PROP_WIDTH, 0, WIDTH_CALC), UNIT_PX, css__calc_function(c, token));
	} else {
		css_fixed length = 0;
		uint32_t unit = 0;
		*ctx = orig_ctx;

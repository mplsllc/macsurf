...
uint8_t css__calc_slot_for_prop(uint32_t prop)
{
	...
	case CSS_PROP_FONT_SIZE:		return 29;
	default:				return 0xFF;
	}
}

/**
 * fixes1166: recover the computed-style unit for a calc-family
 * expression.
 *
 * The expression bytecode (an interned lwc string) is executed linearly
 * and ends with CALC_FINISH; the parser emits a terminal MIN/MAX/CLAMP
 * operator for those functions, so the last opcode before the finish
 * marker records which function produced the expression. Everything
 * else is a plain calc().
 *
		...
		css_error (*fun)(css_computed_style *, uint8_t, css_color))
{
...

	{
		...
		/* fixes1266 (#167) - fixes1265's `fun == set_background_color`
		 * filter matched ZERO declarations for body on real hardware,
		 * despite fixes1264 proving body_bg computes to a real,
		 * non-initial colour - meaning something IS winning this
		 * cascade. Root cause: set_background_color is `static
		 * inline` (autogenerated_propset.h). A static function has
		 * no single canonical address across translation units in
		 * C; background_color.c's compiled instance and the one
		 * visible from THIS file are not guaranteed to be the same
		 * machine address, so the pointer comparison could be
		 * (and evidently was) always false. getOpcode(opv) is a
		 * pure integer extracted from the bytecode word itself -
		 * CSS_PROP_BACKGROUND_COLOR is the same enum value
		 * regardless of which TU asks - so this has no equivalent
		 * cross-TU risk. */
		if (getOpcode(opv) == CSS_PROP_BACKGROUND_COLOR &&

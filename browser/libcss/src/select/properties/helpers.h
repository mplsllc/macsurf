...
/* fixes1299 (#167) - records the specificity of the most recent WINNING
 * write to each calc-expression slot, and how many times that slot has
 * been written in total (process lifetime, not per-element -- this is a
 * coarse regression-test signal, not a per-style counter). Called from
 * the five shared calc-capable cascade helpers below, ONLY when a
 * declaration has already been confirmed to win css__outranks_existing()
 * -- see the split-brain fix in css__cascade_length_auto and friends.
 * Real production code (not test-only #ifdef'd), because the whole point
 * is proving what the ACTUAL shipped cascade path does, not a
 * reimplementation; the cost is one array write, cheap enough for OS 9.
 * Exposed to the Linux harness (cssprobe.c) via
 * css__calc_slot_write_test_count/spec below. */

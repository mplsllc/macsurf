		...
		css_select_state *state)
{
	...
	if (css__outranks_existing(getOpcode(opv), isImportant(opv), state,
			getFlagValue(opv))) {
		return set_text_align_last(state->computed, (uint8_t)value);
	}
...
css_error css__copy_text_align_last(
		...
		css_computed_style *to)
{
	...
	return set_text_align_last(to, get_text_align_last(from));
}
		...
		css_computed_style *result)
{
	...
	return css__copy_text_align_last(
			type == CSS_TEXT_ALIGN_LAST_INHERIT ? parent : child,
			result);
}

uint32_t destroy_text_align_last(void *bytecode)
{
	(void)bytecode;
	return 0;
}

		...
		css_select_state *state)
{
	...
	if (css__outranks_existing(getOpcode(opv), isImportant(opv), state,
		...
		if (hasFlagValue(opv) && getFlagValue(opv) == FLAG_VALUE_INHERIT) {
			...
			if (d != NULL) {
				d->inherit_flags |= CSS_TRANS_INHERIT_DUR;
			}
			...
			if (d != NULL) {
				d->inherit_flags &= ~CSS_TRANS_INHERIT_DUR;
				d->duration_count = count;
				for (i = 0; i < count; i++) {
					d->durations[i] = durations[i];
				}
			}
		}
	}
...
css_error css__copy_transition_duration(
...
uint32_t destroy_transition_duration(void *bytecode)
{
	(void)bytecode;
	return 0;
}

...
css_error css_computed_style_destroy(css_computed_style *style)
{
	...
	/* fixes118: free heap-allocated grid track array if any. */
	...
	if (style->transition_data != NULL) {
		css__free_transition_data(style->transition_data);
		style->transition_data = NULL;
	}
		...
		css_computed_style **clone_out)
{
	...
	if (orig->transition_data != NULL) {
		clone->transition_data = css__clone_transition_data(orig->transition_data);
	}
		...
		css_computed_style **restrict result)
{
	...
	/* Group 2 (#322): transition longhands are non-inherited by default.
	 * If child specified inherit on a longhand, inherit_flags indicates which
	 * longhands copy from parent. */
	if (child->transition_data != NULL || (parent != NULL && parent->transition_data != NULL)) {
		css_computed_transition_data *d;
		if (child->transition_data != NULL) {
			composed->transition_data = css__clone_transition_data(child->transition_data);
		} else {
			/* Child had no transitions declared; stays NULL unless explicitly inheriting */
			composed->transition_data = NULL;
		}

		d = composed->transition_data;
		if (d != NULL && d->inherit_flags != 0 && parent != NULL && parent->transition_data != NULL) {
			const css_computed_transition_data *pd = parent->transition_data;
			uint8_t k;
			if (d->inherit_flags & CSS_TRANS_INHERIT_PROP) {
				for (k = 0; k < d->prop_count; k++) {
					if (d->props[k].kind == CSS_TRANS_PROP_CUSTOM_IDENT && d->props[k].custom_name != NULL) {
						lwc_string_unref(d->props[k].custom_name);
						d->props[k].custom_name = NULL;
					}
				}
				d->prop_count = pd->prop_count;
				for (k = 0; k < pd->prop_count; k++) {
					d->props[k] = pd->props[k];
					if (d->props[k].kind == CSS_TRANS_PROP_CUSTOM_IDENT && d->props[k].custom_name != NULL) {
						lwc_string_ref(d->props[k].custom_name);
					}
				}
			}
			if (d->inherit_flags & CSS_TRANS_INHERIT_DUR) {
				d->duration_count = pd->duration_count;
				for (k = 0; k < pd->duration_count; k++) {
					d->durations[k] = pd->durations[k];
				}
			}
			if (d->inherit_flags & CSS_TRANS_INHERIT_TIMING) {
				d->timing_count = pd->timing_count;
				for (k = 0; k < pd->timing_count; k++) {
					d->timings[k] = pd->timings[k];
				}
			}
			if (d->inherit_flags & CSS_TRANS_INHERIT_DELAY) {
				d->delay_count = pd->delay_count;
				for (k = 0; k < pd->delay_count; k++) {
					d->delays[k] = pd->delays[k];
				}
			}
			d->inherit_flags = 0; /* resolved */
		} else if (d != NULL) {
			d->inherit_flags = 0;
		}
	}
...
int32_t css_computed_box_shadow_3(const css_computed_style *style)
{
	return style->box_shadow_3;
}

int32_t css_computed_macsurf_hstripe_bg(const css_computed_style *style)
{
...
uint8_t css_computed_macsurf_text_shadow(const css_computed_style *style,
		int32_t *packed)
{
	return get_macsurf_text_shadow(style, packed);
}

uint8_t css_computed_macsurf_transform(const css_computed_style *style,
		int32_t *packed)
{
...
int32_t css_computed_macsurf_grid_placement(
		const css_computed_style *style)
{
	/* fixes158: raw packed placement, see autogenerated_computed.h. */
...
uint8_t css_computed_macsurf_animation_rotate(
		const css_computed_style *style, int32_t *packed)
{
	return get_macsurf_animation_rotate(style, packed);
}
...
/******************************************************************************
 * CSS Transitions Accessors (Group 2, #322)                                  *
 ******************************************************************************/
...
uint32_t css_computed_transition_descriptor_count(
		const css_computed_style *style)
{
	if (style == NULL || style->transition_data == NULL) return 0;
	if (style->transition_data->prop_count == 1 &&
		style->transition_data->props[0].kind == CSS_TRANS_PROP_NONE) {
		return 0;
	}
	return style->transition_data->prop_count;
}
...
{
	const css_computed_transition_data *d;
	if (style == NULL || out == NULL) return false;
	d = style->transition_data;

	if (d == NULL) {
		/* Default: transition-property: all, duration: 0s, timing: ease, delay: 0s */
		if (index != 0) return false;
		out->prop.kind = CSS_TRANS_PROP_ALL;
		out->prop.prop_id = 0;
		out->prop.custom_name = NULL;
		out->duration = 0;
		out->timing.type = CSS_TIMING_EASE;
		out->timing.x1 = (int32_t)(0.25 * 65536.0);
		out->timing.y1 = (int32_t)(0.1 * 65536.0);
		out->timing.x2 = (int32_t)(0.25 * 65536.0);
		out->timing.y2 = (int32_t)(1.0 * 65536.0);
		out->timing.step_count = 0;
		out->timing.step_pos = 0;
		out->delay = 0;
		return true;
	}

	if (d->prop_count == 1 && d->props[0].kind == CSS_TRANS_PROP_NONE) {
		return false;
	}

	if (index >= d->prop_count) return false;

	/* Property: exactly index */
	out->prop = d->props[index];

	/* Duration: modulo repetition */
	if (d->duration_count > 0) {
		out->duration = d->durations[index % d->duration_count];
	} else {
		out->duration = 0;
	}

	/* Timing: modulo repetition */
	if (d->timing_count > 0) {
		out->timing = d->timings[index % d->timing_count];
	} else {
		out->timing.type = CSS_TIMING_EASE;
		out->timing.x1 = (int32_t)(0.25 * 65536.0);
		out->timing.y1 = (int32_t)(0.1 * 65536.0);
	...
	/* Delay: modulo repetition */
	if (d->delay_count > 0) {
		out->delay = d->delays[index % d->delay_count];
	} else {
		out->delay = 0;
	}
...
{
	const css_computed_transition_data *da = (a != NULL) ? a->transition_data : NULL;
	const css_computed_transition_data *db = (b != NULL) ? b->transition_data : NULL;
	uint8_t i;

	if (da == db) return true;
	if (css__is_default_transition_data(da) && css__is_default_transition_data(db))
		return true;
	...
	for (i = 0; i < da->prop_count; i++) {
		if (da->props[i].kind != db->props[i].kind) return false;
		if (da->props[i].kind == CSS_TRANS_PROP_KNOWN &&
			da->props[i].prop_id != db->props[i].prop_id) return false;
		if (da->props[i].kind == CSS_TRANS_PROP_CUSTOM_IDENT) {
			bool match = false;
			if (da->props[i].custom_name == db->props[i].custom_name) match = true;
			else if (da->props[i].custom_name != NULL && db->props[i].custom_name != NULL) {
				if (lwc_string_isequal(da->props[i].custom_name,
					db->props[i].custom_name, &match) != lwc_error_ok || !match)
					return false;
			} else return false;
		}
	}

	for (i = 0; i < da->duration_count; i++) {
		if (da->durations[i] != db->durations[i]) return false;
	}

	for (i = 0; i < da->timing_count; i++) {
		if (da->timings[i].type != db->timings[i].type) return false;
		if (da->timings[i].type == CSS_TIMING_CUBIC_BEZIER) {
			if (da->timings[i].x1 != db->timings[i].x1 ||
				da->timings[i].y1 != db->timings[i].y1 ||
				da->timings[i].x2 != db->timings[i].x2 ||
				da->timings[i].y2 != db->timings[i].y2) return false;
		} else if (da->timings[i].type == CSS_TIMING_STEPS) {
			if (da->timings[i].step_count != db->timings[i].step_count ||
				da->timings[i].step_pos != db->timings[i].step_pos) return false;
		}
	}

	for (i = 0; i < da->delay_count; i++) {
		if (da->delays[i] != db->delays[i]) return false;
	}

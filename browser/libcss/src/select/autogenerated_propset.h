...
static inline css_error set_border_radius(css_computed_style *style, uint8_t
		type, css_fixed length, css_unit unit)
{
	uint32_t *bits = &style->i.bits[BORDER_RADIUS_INDEX];
	
	/* 7bits: uuuuutt : unit | type */
	*bits = (*bits & ~BORDER_RADIUS_MASK) | ((((uint32_t)type & 0x3) | (
			unit << 2)) << BORDER_RADIUS_SHIFT);
	
	style->i.border_radius = length;
		...
		int32_t integer)
{
	...
	/* 2bits: tt : type */
	*bits = (*bits & ~BOX_SHADOW_MASK) | (((uint32_t)type & 0x3) <<
			BOX_SHADOW_SHIFT);
	
	style->i.box_shadow = integer;
...
static inline css_error set_macsurf_gradient(css_computed_style *style, uint8_t
		type, int32_t integer)
{
	uint32_t *bits = &style->i.bits[MACSURF_GRADIENT_INDEX];
	
	/* 2bits: tt : type */
	*bits = (*bits & ~MACSURF_GRADIENT_MASK) | (((uint32_t)type & 0x3) <<
			MACSURF_GRADIENT_SHIFT);
	
	style->i.macsurf_gradient = integer;
		...
		uint8_t type)
{
	...
	*bits = (*bits & ~TEXT_OVERFLOW_MASK) |
			(((uint32_t)type & 0x3) << TEXT_OVERFLOW_SHIFT);

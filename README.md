...
<p>
  MacSurf is a web browser for Classic Mac OS 9 PowerPC. CSS3, ES5 JavaScript, PNGs with alpha — running on a beige G3.
</p>
...
>
> First numbered release was **0.1a1** in May 2026. Latest is **[v1.2 — Sealed](https://github.com/mplsllc/macsurf/releases/latest)** (2026-05-29): the security release. **macTLS v1.0 (macEntropy v1.0)** replaces the documented insecure-stub entropy source with a SHA-256 accumulator feeding BearSSL's HMAC-DRBG — fed by OT packet-arrival jitter, mouse and key-press timing, high-res clock samples, and a Preferences-folder seed file persisted across boots. Plus **POST forms** actually go out (both fetchers were silently dropping the body), a real **download manager** with NavPutFile save dialog and Content-Disposition: attachment handling for Drupal-style sites, and a fix for the HTTPS-abort crash that surfaced on duckduckgo.com. Full notes in [docs/release-notes/MacSurf-1.2.md](docs/release-notes/MacSurf-1.2.md). Predecessor [v1.0 — Showcase](https://github.com/mplsllc/macsurf/releases/tag/v1.0) (2026-05-28) is in [docs/release-notes/MacSurf-1.0.md](docs/release-notes/MacSurf-1.0.md).
...
Latest is **[MacSurf v1.2 — Sealed](https://github.com/mplsllc/macsurf/releases/tag/v1.2)** (2026-05-29): the security release. macTLS v1.0 (macEntropy v1.0) closes the documented insecure-stub entropy hole with a SHA-256 accumulator feeding HMAC-DRBG. POST forms now work end-to-end; a real download manager landed with NavServices save dialog and Content-Disposition: attachment routing; the HTTPS-abort crash that surfaced on duckduckgo.com is fixed. Predecessor [v1.0 "Showcase"](https://github.com/mplsllc/macsurf/releases/tag/v1.0) was the chrome-redesign release; [v0.6.2 "Speed-Run"](https://github.com/mplsllc/macsurf/releases/tag/v0.6.2) was the cold-load speedup (mactrove.com 30+s → ~2-3s); first numbered alpha at [v0.1a1](https://github.com/mplsllc/macsurf/releases/tag/v0.1a1).

- **[MacSurf.sit](https://github.com/mplsllc/macsurf/releases/download/v1.2/MacSurf.sit)** — the v1.2 binary, ready to run. Expand on Mac OS 9.1+ with CarbonLib 1.5+ and launch.
- Building from source: clone the repo, then on the Mac side open `browser/netsurf/frontends/macos9/MacSurf.mcp` in CodeWarrior 8 and choose Build. v1.2 builders on a 1.0 workspace need to add `desktop/download.c` to the project file list (introduced as part of the download manager — provides `download_context_create` and the four `download_context_get_*` accessors). The earliest release also ships a [BuildPack.sit](https://github.com/mplsllc/macsurf/releases/download/v0.1a1/MacSurf-BuildPack.sit) snapshot with the CW8 project pre-wired, but current builds work straight from a fresh clone.
